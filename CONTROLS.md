...
| **`C-SPORT-SHADOW`** (2026-09-26(c)) | The same lane for every other sport: after the card is frozen and before the start, `sport_models.py shadow` freezes A0/A1 for the event; settled from the ESPN feed; never on or changing a card; reviewed at 150 settled rows per league. Validation where public results existed: `research/sport_models_2026-09-26/README.md` | **OPERATIVE** (measurement, shadow) | `evidence_status.py` prints per-league counts |
