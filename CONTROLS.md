...
## 2026-09-26 — evidence-before-rules controls (review implementation)

Home: `RULES_GENERAL.md` §"2026-09-26". Tools: `tools/evidence_status.py`, `tools/slate_universe.py`, `tools/market_benchmark.py`, `tools/mlb_model.py`, `tools/make_manifest.py --category`. None of these is a forecasting coefficient, cap or ranking override.

| Control | Rule in one line | Class | Failure consequence |
|---|---|---|---|
| **`C-RULE-FREEZE`** | Until `C-BASELINE-SKILL` and `T-RM1-PROSPECTIVE` reach checkpoint, governance changes are validity repairs, retrieval/integrity, measurement/disclosure or documentation only; one manifest per issuing day; a `MODEL_CHANGE` needs the user's explicit instruction | **OPERATIVE** (governance; enforced by `make_manifest.py`) | Manifest refused |
| **`C-READING-GATE`** | Tier 1 = `CURRENT_RULES.md` + the sport file's §0 live page + log snapshot + guide §19; everything else by citation. A control added to a sport file is added to its §0 page in the same pass | **OPERATIVE** (documentation) | Documentation defect; fix in the same pass |
| **`C-EVENT-UNIVERSE`** | Declare the day's events before carding (`slate_universe.py declare`); card each or record a skip reason; cards print `UNIVERSE:` or `OUT_OF_UNIVERSE`; scorecards split the two | **OPERATIVE** (measurement) | `MISSING` events reported; `T-UNIVERSE-VS-SELECTED` |
| **`C-MARKET-BENCHMARK`** | After settlement only, the operator records the closing no-vig probability; entered-before-settled rows are excluded; no raw odds stored; never read while building a card, never cited by a rule | **OPERATIVE** (measurement, scoring-only) | Row excluded; a card citing it is a process defect |
| **`C-MLB-SHADOW`** | After an MLB card is frozen and before first pitch, `mlb_model.py shadow` freezes the pilot's forecast; it never appears on or changes a card; reviewed at 150 settled games | **OPERATIVE** (measurement, shadow) | Missing shadow rows are reported by `evidence_status.py` |
| **`C-SPORT-SHADOW`** (2026-09-26(c); tennis and cricket lanes and audit `10s` 2026-09-26(d)) | The same lane for every other sport, tennis and cricket included: after the card is frozen and before the start, `sport_models.py shadow` freezes A0/A1 for the event. It is blind (row ID only), settled from the ESPN feed, and never on or changing a card. Each settlement prints `SHADOW: <row id>`, `NO_LANE` or `MISSED` (audit `10s`, strict). Reviewed at 150 settled rows per league. Validation: `research/sport_models_2026-09-26/README.md` | **OPERATIVE** (measurement, shadow) | `evidence_status.py` per-league counts; `audit_card_controls.py --settlement --strict` (`10s`) |
| **`C-MODEL-ANCHOR`** (2026-09-26(e)) | `python tools/model_anchor.py registry` derives each league's reference model per target (A1S, A1, TB-1 or POP), applying the preregistered P4 rule to held-out intervals stored in the tool. It is the reference for shadow-lane reviews and the predictability map. It is never printed on, cited by or used to revise a card; status `REFERENCE` (P3: the cards were no worse than A1 on their own contracts). `ACTIVE` needs the user's `MODEL_CHANGE` instruction | **OPERATIVE** (measurement, reference) | `tools/test_model_anchor.py` fails if `STATUS` is `ACTIVE` without the rules saying so; a card citing it is a process defect |
| **`C-PREDICTABILITY-MAP`** (2026-09-26(e)) | The card prints its league's `BASE_RATES_REGISTER.md` §7.8 row (the share of STRONG favourites and their win rate) beside the track-record row, or `NOT_YET_DERIVED`. Under `TOP2_COIN_FLIP` in a league with few or no STRONG favourites, the delivery says the slate cannot produce a STRONG Rank 1. `SLATE_ADVISORY` and `C-EVENT-UNIVERSE` may use the map, declared before research. It moves no probability or rank | **OPERATIVE** (disclosure) | Disclosure defect; fix on the next card |
| **`T-RM1-PROSPECTIVE` (amended)** | Cushion-class rows reported separately; the cushion term reverts to disclosure if those rows beat their q by more than 0.15 over 15 rows | validity | Term reverts |
