...
Control revision: **CR-2026.09.21-3** — preserves CR-2026.09.21-2 and removes residual live-rule/version/queue drift found by post-reconciliation read-back. Freeze the SHA-256 file receipt from [CONTROL_MANIFEST_2026-09-27.md](CONTROL_MANIFEST_2026-09-27.md) with every new card. It is the 2026-09-26(e) predictability receipt (category MEASUREMENT, with the TB-1 validity repairs for NRL sides and NFL totals; no forecasting coefficient, cap or ranking override changed): the predictability map `C-PREDICTABILITY-MAP`, the reference registry `C-MODEL-ANCHOR` (`tools/model_anchor.py`, status `REFERENCE`), `T-FAV70-BAND`, `T-MLB-V2-2025` concluded, and C1: no model change meets the user's 2026-09-27 bar (`RULES_GENERAL.md` §"2026-09-26(e)"(g)). It re-hashes [CONTROL_MANIFEST_2026-09-26.md](CONTROL_MANIFEST_2026-09-26.md), the 2026-09-26 review-implementation receipt (category MEASUREMENT; no forecasting coefficient, cap or ranking override changed): `C-RULE-FREEZE`, `C-READING-GATE` with the sport files' §0 live pages, `C-EVENT-UNIVERSE`, `C-MARKET-BENCHMARK` (post-settlement only), `C-MLB-SHADOW`, `tools/evidence_status.py` and `LEARNINGS_INDEX.md` (`RULES_GENERAL.md` §"2026-09-26"), plus (2026-09-26(c), (d)) the numerical models for every other sport and their blind shadow lane `C-SPORT-SHADOW` (`tools/sport_models.py`; tennis and cricket included; settlement record audit `10s`; shadow only, never a card input). It builds on [CONTROL_MANIFEST_2026-09-25-6.md](CONTROL_MANIFEST_2026-09-25-6.md), which re-hashes [CONTROL_MANIFEST_2026-09-25-5.md](CONTROL_MANIFEST_2026-09-25-5.md), the Rank-1/Rank-2 content receipt (2026-09-25(e)) under the same control revision, after the 2026-09-26(a) canonical-ID assignment (P-516, P-517; README, CHANGELOG and the settled-row extractor). No card was issued under -5. It adds `C-RANK-MODEL` (RM-1, `tools/rank_model.py`), `C-TOP2-QUALITY`, `C-TEAM-BASELINE` (TB-1, `tools/team_baseline.py`), the amended `C-PLUS-CUSHION`, `C-SETTLEMENT-FROM-FEED` and the NFL/AFL/NRL references to everything in [CONTROL_MANIFEST_2026-09-25-4.md](CONTROL_MANIFEST_2026-09-25-4.md). That manifest was the post-settled-row-review receipt and remains the receipt for P-510–P-515. It added the 2026-09-25(d) calibration controls and construction tools (`C-PLUS-CUSHION`, `C-DEPARTURE-LEDGER`, `C-TRACK-RECORD`, `C-LOW-RESOLUTION-BAND`, `tools/card_math.py`, `tools/calibration_report.py`) to the 2026-09-25(c) baseline-skill, current-rules and repository controls, and the earlier 2026-09-24(f)/2026-09-25 controls. Verify it with `python tools/verify_manifest.py`. [CONTROL_MANIFEST_2026-09-25-3.md](CONTROL_MANIFEST_2026-09-25-3.md), [CONTROL_MANIFEST_2026-09-25-2.md](CONTROL_MANIFEST_2026-09-25-2.md) and [CONTROL_MANIFEST_2026-09-25.md](CONTROL_MANIFEST_2026-09-25.md) were the receipts of the repository review, the research pass and the audit closure (no card was issued under any of them); [CONTROL_MANIFEST_2026-09-23.md](CONTROL_MANIFEST_2026-09-23.md) remains the receipt for P-495–P-509; [CONTROL_MANIFEST_2026-09-21-3.md](CONTROL_MANIFEST_2026-09-21-3.md) for cards issued before 2026-09-23 about 22:00 AEST.
...
**After settlement only (`C-MARKET-BENCHMARK`, 2026-09-26).** The one permitted use of market prices is a post-settlement scoring comparison that never reaches a forecast. The operator records the closing no-vig probability for a settled decision in `MARKET_BENCHMARK_LEDGER.md`, entered after settlement, with no raw odds stored. Agents never fetch betting sites, and no rule, rank, width or retrospective may cite the ledger (`RULES_GENERAL.md` §"2026-09-26"(d)). Nothing in this section's pre-freeze prohibition changes.
...
[NUMERICAL_PROGRAM.md](NUMERICAL_PROGRAM.md) controls implementation scope and build status. **2026-09-26:** the MLB A0/A1 pilot now exists as tested code (`tools/mlb_model.py`, `C-MLB-SHADOW`). It is not fitted and not validated. It runs only as a prospective shadow after an MLB card is frozen, and it is never a card input until its review (`RULES_GENERAL.md` §"2026-09-26"(e)). The user's 17 September implementation request authorizes the audit changes; the later clarification prioritizes implementation within Markdown. Algorithms, input schemas and acceptance tests are therefore supplied in Markdown. No new approval is requested for these authorized changes. Real data fitting, empirical quality approval, calibration and prospective evidence are not fabricated as document-editing milestones. **2026-09-26(c):** every other sport now has a reduced-feature A0/A1 build in `tools/sport_models.py` with its own shadow lane (`C-SPORT-SHADOW`). These builds are validated on public results where reachable (`research/sport_models_2026-09-26/README.md`) and are never a card input.


## 9. Authority and precedence


Current user instructions take precedence. Within the documents: honesty/identity/anti-hindsight invariants; this method and SCORING_AND_VALIDATION; current detailed source/target gates and sport-specific rules; then promoted process controls. Predictions, dated retrospectives, audit snapshots and archived text are evidence, not active instructions. No historic paragraph can reinstate retired caps, the pseudo-trend test, market features or settlement-only eligibility. The active log's top snapshot alone controls queues and next ID. `CURRENT_RULES.md` (2026-09-25(c)) and each sport file's §0 live rules page (2026-09-26, `C-READING-GATE`) are derived summaries and maps. Where it disagrees with the section it cites, the cited section governs and the summary is corrected. `CHANGELOG.md` is history, not instruction.
