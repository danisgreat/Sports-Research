...
When no usable prices are supplied, rank by marginal estimated chance and robustness of settling as a win under the exact contract. Lower loss probability and evidence quality are frozen tie-breakers only; dependence does not turn the ordinal ranking into a hedge. This is **not** an expected-value, staking, profitability, or diversification ranking. When prices are captured, value analysis is a separate field and must compare the model with the same-time, same-contract, de-vigged market baseline.
...
| Calibration, validation, accuracy, edge, or profit claims | **PROHIBITED** |

Special roles remain explicit: P-021 contributes administration/state logic only; the unsettleable P-003 corner row contributes source and settlement lessons only; P-057 contributes retrospective mechanism evidence but is tagged **LATE_IMPORT** and is excluded from prospective performance. Multiple views or contracts from one event are not independent training examples.
...
- reserve only demonstrably pre-result P-061 onward cards as **E1-Q**, the prospective qualitative process/rank cohort under the current non-numeric champion; P-061–P-063 are `E1-Q-LATE_IMPORT` because their first available combined artifact post-dates the finals and therefore contribute descriptive process evidence only;
...
Every forecast follows this order.
...
4. **Estimate dynamic strength.** Use opponent-adjusted process data with time decay and explicit regime breaks for coach, role, lineup, rules, venue, injury, or season changes. Sparse estimates shrink toward an appropriate league, team, role, or position prior.
5. **Model participant exposure.** Estimate who will play and how much: minutes, possessions, drives, plate appearances, innings, balls, overs, wickets, time on ground, centre-bounce attendance, sets, or attacking sequences. Store replacement and late-change branches.
...
7. **Apply context conditionally.** Rest, workload, travel, weather, surface, officials, motivation, and comments change a forecast only through a stated exposure, rate, or tactical mechanism.
...
9. **Map and rank contracts.** Draw each settlement interval, deduplicate aliases, tag dependence, calculate win/push/loss only when a genuine numerical model exists, and rank all supplied unresolved rows by marginal win likelihood. For derivative/niche markets, require the target-event exposure/rate/opponent/context chain and exact settlement definition before `LEAN`/`SUPPORTED`; otherwise cap at `FORCED RANK` and LOW/MEDIUM-LOW evidence. An optional portfolio never silently changes these ranks.
...
The record artifact itself also needs provenance. Store its path/stable ID, cryptographic hash where available, and first demonstrable creation/publication time. An embedded issue timestamp without a pre-result artifact is insufficient for prospective eligibility; classify it `E1-Q-LATE_IMPORT` until an earlier immutable copy is produced.
...
5. For each supplied contract, integrate the same event distribution over its win, push and loss intervals.
6. When a same-time binary market probability exists, maintain a separate anchored challenger (multi-outcome markets require joint normalisation):

   **anchored log-odds = de-vigged market log-odds + shrunk structural model adjustment**
...
## 8. Market baseline and ensemble policy

Odds are optional evidence, never a gate. When present, store both sides from the same source and timestamp, the exact contract, overround, de-vig method, and comparable closing snapshot. Never use a later close as an earlier input.

Maintain two challenger baselines when data permits:

1. a market-blind sport model; and
2. a market-anchored model using same-time de-vigged probability plus a shrunk structural adjustment.

The market baseline is a benchmark, not truth. Closing-line value is a process diagnostic, not proof of forecast accuracy, value, or profit.

Every build card declares `MARKET_BLIND`, `MARKET_ONLY_BASELINE`, or `MARKET_INFORMED`. These lanes share the same cutoff and event population but are scored separately. Market inputs captured after cutoff are leakage; market inputs captured at cutoff are not leakage, but they invalidate any claim of sports-only independence.

Do not estimate complex ensemble weights from the current log. Start with a frozen simple model or equal/strongly shrunk weights. Adopt stacking or learned weights only when chronological out-of-sample scoring demonstrates improvement.
...
- same-time de-vigged market baseline when available;
...
| Win/push/loss probability | Multiclass Brier and log score |
...
| Market source / price state | NOT CAPTURED if absent |

### Evidence ledger

| Source ID | Data item | Value/unit | Effective | First known/published | Observed | Accessed / feature age | Source/tier/provider version | Transformation/feature version | Predictive tier | Missing/conflict count |
|---|---|---|---|---|---|---|---|---|---|---|
...
| Scenario | Preconditions | Exposure/rate effect | Outcome corridor | Contracts helped | Contracts hurt |
|---|---|---|---|---|---|
...
| View/contract | Official final value | Outcome | Margin to line | Official source | Model score | Market score | CLV status |
|---|---:|---|---:|---|---:|---:|---|

Use only WIN, LOSS, PUSH, VOID, UNRESOLVED, or UNSETTLEABLE as the contract outcome. Never convert an avoided loss into a contract win.
...
`settlement_3way` preserves WIN/PUSH/LOSS. VOID, UNSETTLEABLE, and SETTLED_AT_ISSUE are excluded from predictive scoring under a frozen policy. An alias shares its canonical label. `is_win` may be derived for rank diagnostics as WIN=`1`, otherwise=`0`; a different push relevance is a different objective and method version.
...
| Field | Current state |
|---|---|
| Model/method ID | MDS-2026.08.30-v2.7 |
...
| Prospective qualitative data | Queue IDs and counts are not duplicated here; use the active log named in README and its top controlling snapshot. Late-import and process-defective cohorts remain excluded under §2 |
...
| Numerical training program | NTS-2026.08.25-v0.2 Stage 0 all-sports design/pre-fit; source, H0 and model registries exist only as Markdown specifications |
| Validation status | VALIDATION PENDING for every sport/competition/market/state slice |
| Market-edge status | NOT ESTABLISHED |
...
The 2026-08-25 numerical architecture is separately versioned as `NTS-2026.08.25-v0.2`. Version 0.2 generalises the Stage-0 design to its listed sports, adds systematic event-universe and field-specific source rules, and introduces ice hockey as design-only scope. Tennis, rugby union and rugby sevens remain outside numerical scope until explicit target/source/model cards are approved. MDS v2.7 does not create H0, fit a model, promote a coefficient or publish a probability.
