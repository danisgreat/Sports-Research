...
| Governing method for the next issue | METHOD.md **MDS-2026.09.19-v4.3** / control revision **CR-2026.09.21-3**; SCORING_AND_VALIDATION **SCV-2026.09.19-v2** (§15, RM-1). **Freeze with every card:** `CONTROL_MANIFEST_2026-09-27.md`, SHA-256 `74f34e8d1c9692db3eb4b964332ace91750f881f3d16bc0c0c6acf2868d62b31` (2026-09-26(e) predictability pass, category MEASUREMENT with the TB-1 validity repairs for NRL sides and NFL totals, and C1: no model change meets the user's 2026-09-27 bar; 124 files, CRLF form; supersedes `CONTROL_MANIFEST_2026-09-26.md`, SHA-256 `b6e038d36c09dc295074b2517e9d50b4afb6e760e239ede5d90f487706a7f098`, under which no card was issued. That manifest was the 2026-09-26 review implementation, category MEASUREMENT: rule freeze, event universe, post-settlement market benchmark, MLB shadow model, one-page live rules, and blind numerical shadow models for every sport with the settlement `SHADOW:` record; 120 files, CRLF form; supersedes `CONTROL_MANIFEST_2026-09-25-6.md`, under which no card was issued). Every card from this manifest also carries the `UNIVERSE:` field (`C-EVENT-UNIVERSE`; declare with `python tools/slate_universe.py declare`). Verify it with `python tools/verify_manifest.py`. |
...
**Added 2026-09-26(e) (`CONTROL_MANIFEST_2026-09-27.md`; `RULES_GENERAL.md` §"2026-09-26(e)"):**

12. **NRL sides and NFL totals now print `TB1_NO_RESOLUTION`** and anchor on `BASELINE_P` (validity repair; item 2's covered list is narrowed accordingly).
13. **Print the league's predictability row** (`BASE_RATES_REGISTER.md` §7.8: share of STRONG favourites and their win rate, or `NOT_YET_DERIVED`) beside the track-record row (`C-PREDICTABILITY-MAP`). Under `TOP2_COIN_FLIP` in a league with few or no STRONG favourites (MLB, NHL, soccer three-way, most totals), say the slate cannot produce a STRONG Rank 1.
14. **`tools/model_anchor.py` is a reference, never a card input** (`C-MODEL-ANCHOR`, status `REFERENCE`). Do not print or cite it on a card.
