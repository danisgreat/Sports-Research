...
`python tools/sport_models.py predict --league <t20|odi> --home … --away … --venue … --cricsheet <dir>` (research only). A1 is Elo for the result plus a ridge batting/bowling/venue first-innings total; second innings are target-censored and not modelled. **Not validated:** no cricket results could be reached. It is never printed, ranked or cited on a card (`RULES_GENERAL.md` §"2026-09-26" (k); `research/sport_models_2026-09-26/README.md`).
