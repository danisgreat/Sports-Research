...
`python tools/sport_models.py shadow --league <atp|wta> --event <ESPN competition id> --date … --card P-### --surface … --best-of … --total <games line> --line <games handicap>` after the freeze (`C-SPORT-SHADOW`; a retirement voids the games rows). Use `predict` for research. On ATP 2023 to January 2026, surface-blended Elo narrowly beat overall Elo on winners. The serve-chain games route lost to the population at v1. After v2 (a match-level gap effect, selected on 2021–22) it is level to slightly better. WTA needs local results files. Record it after the freeze and before the start; it is never printed, ranked or cited on a card, and a promotion needs its 150-row review and your instruction (`RULES_GENERAL.md` §"2026-09-26" (k), (l); `research/sport_models_2026-09-26/README.md`).

**Predictability (2026-09-26(e)).** Tennis winners are predictable by Elo (Brier 0.222 against a coin flip's 0.25), and A1 improves that only narrowly. The WTA is not validated (`research/predictability_2026-09-26/README.md`; `BASE_RATES_REGISTER.md` §7.8).
