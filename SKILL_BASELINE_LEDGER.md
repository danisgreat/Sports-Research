...
6. **Report with** `python tools/skill_baseline.py`: paired Brier (card − baseline; negative means the card is better), by family, with a bootstrap interval that resamples whole cards. From 2026-09-26 the tool reports the **Prospective rows** section (which counts toward rule 7) separately from the **Seed rows** (which never count), with progress against the checkpoint; `--section prospective|seed` prints one. The closing-line comparison is the separate `MARKET_BENCHMARK_LEDGER.md`.
