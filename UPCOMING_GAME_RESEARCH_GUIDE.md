...
- [ ] **Other sports (2026-09-26(c), (d), `C-SPORT-SHADOW`):** after the freeze and before the start, ran `python tools/sport_models.py shadow --league <key> --event <ESPN id> --date <date> --card P-### --total <line> --line <home handicap>`. Tennis adds `--surface`/`--best-of`; cricket adds `--espn-path cricket/<id> --cricsheet <dir>`. The output is not on the card, and the command prints only the row ID.
- [ ] **At settlement (audit `10s`):** printed `SHADOW: <row id>`, `SHADOW: NO_LANE <reason>` or `SHADOW: MISSED <reason>`.
