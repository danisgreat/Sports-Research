...
## Cross-sport event-verification source requirement — CR-2026.09.19-4
...
### 6A. Cricket conditions ladder — at least two independent signals

Under RULES_CRICKET.md §2 every card records `STRIP STATUS` (the pitch report) and `MATCH CONDITIONS STATUS`. Effective 2026-09-01 the card must additionally **seek at least two independent conditions signals** from different rungs below, and must always **attempt** rung 6. Two weather pages are not two pitch signals, and several front ends carrying one broadcast quote are one lineage under §2.

**Correction, 2026-09-06(d) (second independent review, finding F18):** the earlier wording claimed rung 6 "cannot fail" / is "always computable." That is false in general — a new venue, a competition with no prior same-format matches, or genuinely insufficient comparable data all break rung 6's own precondition, and a historical venue baseline is not a current-strip inspection regardless. Where rung 6 cannot actually be computed, the honest state is `STRIP STATUS: UNKNOWN`, with an explicitly labelled broader comparable-context prior and correspondingly wider corridor uncertainty — never a fabricated figure to satisfy the "always compute" instruction. A ranking under this uncertainty may still be delivered if requested; it must not claim a pitch certainty the retrieval did not actually establish. Also note: two rungs that trace to the same upstream quote are not independent confirmation of each other, and toss/match-state commentary does not by itself describe the pitch.

**Disclosure is mandatory, effective 2026-09-04.** A 2026-09-04 audit of the two cricket cards issued since this ladder took effect (P-252, P-262 — see PREDICTION_LOG_COMBINED.md's 2026-09-04 general-learning entry) found that both recorded only a bottom-line status (`NOT AVAILABLE` prose in one card; `STRIP STATUS: NOT FOUND AFTER SEARCH` in the other) with no evidence that rungs 1–5 were actually queried, and neither tried the venue+"curator"/"pitch report" search pattern that rung 2/5 already names. A subsequent same-day verification (below) confirmed rung 4 genuinely works for major/franchise cricket. **The rung table itself must therefore appear on the card**, one row per rung, each marked `ATTEMPTED — <exact query/source> — <result>` or `NOT ATTEMPTED`, before `STRIP STATUS: NOT FOUND AFTER SEARCH` may be recorded. A conclusion with no shown attempts fails RULES_CRICKET.md §2.

| Rung | Signal | Lane | Note |
|---:|---|---|---|
| 1 | Toss-time pitch report spoken by captain, curator or named presenter | `SRC-CRIC-RIGHTS-BROADCAST-TOSS-*` | Highest value and hardest to access; capture speaker, wording and match timestamp |
| 2 | Curator or groundstaff statement, venue or board press material, **or a dated venue-profile piece from reputable cricket journalism** | `SRC-CRIC-OFFICIAL-*`, `SRC-CRIC-ICC-MATCH`, host-board pages | Field owner for what it explicitly publishes. Query pattern `"<venue>" curator pitch` / `"<venue>" groundsman` reliably surfaces named, attributable profile pieces even when no toss-day statement exists (worked example: Queen's Park Oval curator Ronald Faria's on-record description of the surface as batting-friendly, found this way on 2026-09-04); caption a profile piece as historical tendency, not confirmed current preparation, and date it |
| 3 | Exact-match facts and toss from a specialist match centre | `SRC-CRIC-SPECIALIST-SCORECARD` — Cricbuzz `cricket-match-facts/{id}/{slug}` verified 2026-09-01 for toss, venue, umpires and local start | Toss and venue are facts; a preview's pitch opinion is not a field-owner fact |
| 4 | Match preview or pitch-and-conditions section from a specialist | ESPNcricinfo preview via the proxy lane in §17 | **Re-verified 2026-09-04** on a live IPL 2026 match-preview URL: direct fetch still returns 403, but `r.jina.ai` returns 200 with genuine, specific, decision-relevant pitch content (named the exact numbered strip in use, its two prior scores, and the head coach's stated expectation of a high-scoring/batting-friendly surface) — this rung is real and works for major/franchise cricket when actually queried, not merely theoretical. Proxy output carries the article's original `Published` time even when the page covers a later/updated event; verify the event date and, where shown, any separate `Updated` time before use |
| 5 | Named reporting quoting the curator or team management | `SRC-REPORTING-NAMED-*` | Name author, outlet and time; downgrade once an official statement exists |
| 6 | **Venue and format historical scoring baseline by innings order** | Official scorecards, board-branded scorecards, `SRC-CRIC-CRICSHEET-JSON` | **Always computable, and therefore mandatory.** First-innings and second-innings distributions at that venue, in that format and rules era, with the sample size stated |

Rung 6 exists because rungs 1–5 can all fail. The log records cards that discussed a total with no venue scoring baseline at all and were wrong in opposite directions at two grounds in the same competition, and cards where the strip was recorded `NOT FOUND` yet a directional total still ranked first. When no strip report is obtainable, the correct output is the rung-6 baseline plus a disclosed evidence limit and the §11.5 low-evidence cap — never an invented surface description and never a weather report standing in for a strip report. For a genuinely sparse, associate, or minor domestic fixture (for example an inaugural European T20 league match), `NOT FOUND AFTER SEARCH` after a shown rung 1–5 attempt is the honest, correct output, not a process failure — the process failure is only ever an unshown or skipped attempt.

Fantasy-cricket and tipping sites publishing generic daily "pitch reports" are not a rung and are never cited, even for corroboration. Confirmed examples excluded on 2026-09-04 inspection, in addition to any earlier-excluded domains: `cricklive.in`, `pitch-report.com`, `crickonly.in`, `cricketstadiumsinfo.com`, `thecricscope.com`, `jaipurcircle.com`, `cricjosh.in`, `cricketfastliveline.in`, `bjsports.live`. The reliable tell is an unattributed "Today's Pitch Report" / "Dream11 tips" style page covering every match on the day's slate with formulaic batting-or-bowling verdicts and no named curator, reporter, or dated on-the-record quote; treat any newly encountered page matching that pattern the same way without registering it here first.
