...
| **G20.2** | **Aggregate upper- and lower-tail budget (revised 2026-09-06(d) — see the corrections below).** For every Under row, using the L10 window already retrieved at `G13.1`, compute and print three sums: `T_hi_fav` = favourite's second-highest L10 scoring output + underdog's L10 median; `T_hi_dog` = underdog's second-highest + favourite's median; `T_hi_both` = both sides' second-highest. For every Over row compute the symmetric lower-tail sums from each side's second-lowest L10 output. | `GATE-TAIL` (**disclosure only**). If any sum falls on the losing side of the line, the row is `TAIL_EXPOSED` and the card writes one sentence on whether a currently active mechanism makes that stress sum less likely than its plain sum suggests. **This sum is a labelled sensitivity scenario, not a claim that the combination was ever jointly observed** — each component may itself have occurred, but summing two different matches' order statistics does not establish that the *combination* has any particular joint probability; do not describe it as "an already-observed combination." **`TAIL_EXPOSED` is disclosure only and imposes no ordinal restriction on its own** — the former hard bar ("the row may not be Rank #1") is withdrawn per the predictive-weighting firewall (`L-087`) applied to this framework's own gates: a same-cohort-derived ordinal rule is exactly what that firewall exists to catch, regardless of whether the framework itself or an external source proposed it. Any ordinal consequence of `TAIL_EXPOSED` is governed solely by the frozen `C-TAIL-BUDGET` prospective test in `LEARNING_REGISTER.md` §3A, which as of 2026-09-06(d) has not completed. |
| **G21.1** | **Total-row path geometry (revised 2026-09-06(d) — corrects a category error).** Classify every total and phase-total row into exactly one of: **`TRUE_UNION`** — settles WIN if *any one* of N enumerable, discrete atomic events occurs, with no summation required (e.g. "at least one goal in the match", "at least one ace"); **`LOW_BAR_CUMULATIVE`** — a cumulative sum must still clear a threshold, but that threshold sits well below the population's typical range, making the row easy to clear without being a mathematical union (a powerplay Over set far below the phase median is this, *not* `TRUE_UNION` — it still requires enough runs to accumulate, event by event, exactly like an Under does); or **`CENTRAL_BAND`** — the line sits inside the corridor's central mass. Print N only for `TRUE_UNION` rows. | `GATE-GEOMETRY` (**disclosure only**). **Corrected 2026-09-06(d):** the prior wording called any low-threshold cumulative total a "union," which is a category error — `1st Half Over 0.5 goals` is a true union (any one goal anywhere in the window wins it), but a low powerplay-runs threshold is still an intersection-type cumulative constraint that merely happens to be easy to clear; conflating the two risked a built-in bias toward whichever side of a total happens to carry a low threshold, which is not a geometric fact, it is a difficulty fact that depends on the actual scoring distribution. **This field is disclosure only and imposes no ordinal restriction** — the former rule ("a `CENTRAL_BAND` row may not outrank a union row") is withdrawn per the same predictive-weighting firewall (`L-087`) applied to `G20.2` above. Any ordinal consequence is governed solely by the frozen `C-PATH-GEOMETRY` test, not completed as of 2026-09-06(d). It does not license promoting an opposite pick to manufacture an Over/Under win, which remains prohibited. |
...
| A row is `DISJOINT` from the Rank-1 implied interval under G25.1 | It may not occupy the top half of the order while Rank #1 stands; repair the tree, re-order, or drop one evidence grade and record the conflict on the card |
...
**G26.1 — top-slot separation floor (revised 2026-09-06(d) — reclassified as `CANDIDATE`, no hard bar).** **Disclosure requirement (mandatory now):** print the row's own descriptive `REFERENCE_BASE_RATE` split against its stated population, and print `NEAR_TIE`/`SMALL`/`MODERATE`/`LARGE` per the `rank_gap` field (`G23`, `L-088`) for every adjacent pair in the order.
...
26. Aggregate tail budget (`G20.2`): `T_hi_fav`, `T_hi_dog`, `T_hi_both` for every Under row and the symmetric lower-tail sums for every Over row, each stated against the line, with the named mechanism sentence for any `TAIL_EXPOSED` row.
27. Total-row path geometry (`G21.1`): `UNION_LOW_THRESHOLD` / `INTERSECTION_CONSTRAINT` / `CENTRAL_BAND` for every total and phase-total row, with N printed for union rows, and the `G26.1` top-slot separation-floor result.
...
7. Participant state per side: lineup `CONFIRMED_OFFICIAL` / `SECONDARY_ONLY` / `NOT_RETRIEVED`, bench, head coach. **Where the competition publishes line-ups before the freeze time** — a cricket XI is nominated before the toss (MCC Law 1.2); baseball, soccer and rugby-league line-ups are normally published before the start — `NOT_RETRIEVED` after publication is recorded as **`RETRIEVAL_MISS`**, a process defect, not as unavailability. `G14.2` still blocks a margin or full-game total from Rank #1 when the bench is not retrieved.
