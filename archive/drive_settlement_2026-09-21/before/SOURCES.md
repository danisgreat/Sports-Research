...
## Universal minimum source and event-state standard — CR-2026.09.19-4
...
### Cricket
`SRC-CRIC-OFFICIAL-*`, `SRC-CRIC-BOARD-BRANDED-SCORECARD`, `SRC-CRIC-ICC-MATCH` — event/format/toss/XI/scorecard/result — `CANDIDATE`. `SRC-CRIC-CRICSHEET-JSON`/`-REGISTER` — historical delivery data, versioned — `CANDIDATE`. `SRC-CRIC-SPECIALIST-SCORECARD`, `SRC-CRIC-PITCH-REPORT` — `CANDIDATE`. `SRC-CRIC-ICC-PITCH-RATING` — official four-tier pitch rating — `CANDIDATE`. `SRC-CRIC-CRICVIZ-PITCHVIZ` — citation-only, numeric index not publicly exposed. Native-language sourcing (`L-067`) applies to non-English-primary boards. Full conditions ladder (six-rung, at least two independent signals): `DATA_SOURCE_REGISTER.md` §6A.
