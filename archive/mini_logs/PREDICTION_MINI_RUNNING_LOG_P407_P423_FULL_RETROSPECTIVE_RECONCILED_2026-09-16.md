# PREDICTION MINI RUNNING LOG — P-407 THROUGH P-423 — RETROSPECTIVE UPDATE

**Status:** ACTIVE LOCAL / EXTERNAL RUNNING LOG — settlement pass completed 2026-09-15  
**Original created:** 2026-09-13T09:59:00Z / 2026-09-13 19:59 AEST  
**Google Drive access:** **READ ONLY** — no Drive files were modified.  
**Method governing this pass:** `MDS-2026.09.06-v4.0` + `RULES_GENERAL §16.9` + relevant `SFA-<SPORT>` files.  
**Evidence status:** `LEARNING_ONLY / NOT PERFORMANCE_ELIGIBLE`.  
**Canonical reconciliation:** fresh Drive check still ends at **P-371** and names **P-372** as next canonical ID. Therefore **none of P-407–P-423 currently collides with a Drive canonical ID**. The issued P-407–P-423 labels are preserved unchanged as local/user-directed working IDs. No temporary canonical alias was required.  
**Immutability:** all original issued selections, ranks, probabilities and pregame source text are preserved below. Settlement appends do not rewrite them.

> `TMP-OPEN-*` entries below are settlement-tracking handles only. They are **not** replacement canonical prediction IDs.

---

## 0. OPEN / PARTIAL / UNRESOLVED — KEEP AT TOP

### 0.1 Local P-407+ fields still not bookable to the frozen evidence standard

| Handle | Parent / row | Remaining field | Current disposition |
|---|---|---|---|
| `TMP-OPEN-20260915-01` | `P-407-C01` | Club Brugge team corners O4.5 | PROVISIONAL RESEARCH WIN; multiple current displays report Club 9 corners; exact frozen current Pro League field-owner row not directly recovered |
| `TMP-OPEN-20260915-02` | `P-408-C01` | Brighton team corners O3.5 | PROVISIONAL RESEARCH LOSS; Guardian/ZeroZero report Brighton 3; exact frozen Premier League/Opta field-owner row not directly recovered |
| `TMP-OPEN-20260915-03` | `P-409-C02` | Troyes team corners O2.5 | PROVISIONAL RESEARCH WIN; SoccerNews/StatMuse report Troyes 5; exact frozen LFP/defined-provider row not directly recovered |
| `TMP-OPEN-20260915-04` | `P-410-C05` | RB Leipzig team corners O4.5 | PROVISIONAL RESEARCH WIN; all recovered displays clear 4.5 but counts differ 7 vs 8; exact provider-specific field not frozen/recovered |
| `TMP-OPEN-20260915-05` | `P-418` | Drukpa FC vs RTC FC — all issued rows | EVENT_STATE_UNVERIFIED; no trustworthy final recovered |
| `TMP-OPEN-20260915-06` | `P-419-C05` | Total corners O7.5 | PROVISIONAL RESEARCH LOSS; two current secondary displays report 2-2 corners; no frozen field owner/provider |

The five corner rows above have a clear *research-result direction* from current displays, but they are deliberately not booked where the card's predeclared provider/field-owner requirement was not met. P-418 is different: even the underlying sporting final is not safely recoverable, so **none** of its rows is graded.

---

### 0.2 P-418 full original block — still open
...
### 2026-09-15 settlement recheck — RESULT NOT VERIFIED / KEEP OPEN

The event cannot be honestly settled. Current RSSSF still lists **Drukpa – RTC** as the 14 September Round-15 fixture without a score, and its table still has both clubs on 14 played. Global Sports Archive likewise continues to display the match as a **next match** rather than a completed result. A lower-tier fixture service also shows conflicting scheduled dates. No BFF/native final or trustworthy current result record was recovered.

Accordingly:
- **no ranked row is graded;**
- **no winner call is graded;**
- **no Brier score is created;**
- the self-generated corner alternate is not reconstructed from unrelated sources;
- the event remains `OPEN / EVENT_STATE_UNVERIFIED`;
- local tracking handle: `TMP-OPEN-20260915-05` (**tracking handle only, not a replacement canonical ID**).

**Process retrospective.** The original card explicitly carried schedule/identity/source limitations. That warning was correct. The new lesson is operational rather than directional: in sparse competitions, pregame source reachability must include a realistic **post-final settlement lane**, not merely a pregame fixture shell.

**Smallest routine change.** Before offering any self-generated derivative in a sparse league, prove that a field-owning or pre-approved result/stat endpoint exists for that competition. If not, withhold the derivative or label it research-only with a terminal source-risk warning.

**Destination if later implemented:** `SOURCES.md`, `DATA_SOURCE_REGISTER.md`, and the sparse-competition/source-reachability portion of `RULES_SOCCER.md`; candidate only, no predictive weight.

**Fresh recheck sources**
- https://www.rsssf.org/tablesb/bhutan2026.html
- https://globalsportsarchive.com/en/soccer/team/royal-thimphu-college-fc/55239/matches
- https://globalsportsarchive.com/en/soccer/team/namlha-fc/55238/matches
- https://bhutanfootball.org/

---

### 0.407 Partial event custody — P-407
...
### 2026-09-15 settlement and retrospective

**Current status:** `FINAL / PARTIAL — derivative Rank(s) 1 remain provisional / NOT BOOKED`  
**Verified sporting result:** Club Brugge 3–1 Royal Antwerp; HT 1–1; current secondary stat records agree on Club 9–4 Antwerp corners.  
**Evidence status:** `LEARNING_ONLY / NOT PERFORMANCE_ELIGIBLE`; original issued ranks/probabilities remain frozen.

| Rank | Frozen issued row | p(win) | Result | Brier | Booking state |
|---:|---|---:|---|---:|---|
| 1 | Club Brugge team corners Over 4.5 | 78% | WIN | — | PROVISIONAL RESEARCH WIN — 9 Club corners reported; frozen current Pro League field-owner record not directly recovered |
| 2 | Royal Antwerp team goals Under 1.5 | 74% | WIN | 0.0676 | WIN |
| 3 | 1H Over 0.5 goals | 72% | WIN | 0.0784 | WIN |
| 4 | Full match Under 3.5 goals | 68% | LOSS | 0.4624 | LOSS |
| 5 | Full match Over 2.5 goals | 56% | WIN | 0.1936 | WIN |

**Definitively booked ranked rows on this card:** 3 W / 1 L; mean row-level Brier **0.2005**. This is an audit statistic only; complementary/nested rows are not independent trials.

**Potential winner:** Club Brugge 60% → **WIN**; potential-winner Brier **0.1600**.

**Result driver and why the rows behaved as they did.** Club's territorial/corner process was directionally strong enough to clear 4.5 in every current stat display recovered, Antwerp scored only once, and the first half was already 1–1. The miss was **Under 3.5**: the 4+ branch that the card itself carried at 32% realised when Club added two second-half goals. That is a distribution-tail realisation, not evidence that the Over should retrospectively have been ranked above every other row.

**Knowability audit.** The high-event path was knowable and explicitly present pregame. What was *not* knowable was the exact sequence of second-half finishing. The corner result is not booked because the card froze the current Pro League match centre as field owner and that exact current event field could not be directly recovered in this pass, even though multiple current displays report 9–4.

**Smallest routine change.** No new directional coefficient. Keep the existing soccer rule that early goals/high conversion can open the full-match goal tail, and strengthen the operational requirement that a derivative row is not booked until its predeclared field owner or a formally accepted fallback is reachable.

**Learning disposition.** Reinforcement, not promotion. Candidate worked example for `RULES_SOCCER.md` goal-state branching and `SOURCES.md`/`DATA_SOURCE_REGISTER.md` derivative-source reachability. The already-identified 2026–27 Belgian competition-format correction still belongs in `LEAGUE_RULES_SOCCER.md`.

**Settlement/retrospective sources added in this pass**
- https://www.soccernews.com/matches/club-brugge-kv-vs-royal-antwerp-fc-jupiler-pro-league-2026-27-560972/?tab=lineups
- https://www.totalcorner.com/h2h/antwerp-vs-club-brugge
- Pro League official 2026-27 match-centre lane was re-probed; the exact current event record was not directly recovered in this pass.

---

### 0.408 Partial event custody — P-408
...
**Current status:** `FINAL / PARTIAL — derivative Rank(s) 1 remain provisional / NOT BOOKED`  
**Verified sporting result:** Coventry 0–5 Brighton; HT 0–1; corners Coventry 5–3 Brighton.  
**Evidence status:** `LEARNING_ONLY / NOT PERFORMANCE_ELIGIBLE`; original issued ranks/probabilities remain frozen.

| Rank | Frozen issued row | p(win) | Result | Brier | Booking state |
|---:|---|---:|---|---:|---|
| 1 | Brighton team corners Over 3.5 | 75% | LOSS | — | PROVISIONAL RESEARCH LOSS — Brighton 3 corners; frozen Premier League/Opta field-owner page not directly recovered |
| 2 | Brighton or Draw | 74% | WIN | 0.0676 | WIN |
| 3 | 1H Over 0.5 goals | 72% | WIN | 0.0784 | WIN |
| 4 | Coventry team goals Under 1.5 | 68% | WIN | 0.1024 | WIN |
| 5 | Full match Over 2.5 goals | 55% | WIN | 0.2025 | WIN |

**Definitively booked ranked rows on this card:** 4 W / 0 L; mean row-level Brier **0.1127**. This is an audit statistic only; complementary/nested rows are not independent trials.

**Potential winner:** Brighton 48% → **WIN**; potential-winner Brier **0.2704**.

**Deep Rank-#1 retrospective.** Brighton won 5–0 with 66% possession and 21 reported attempts, yet produced only **3 corners**. That directly falsified Rank #1 Brighton O3.5 at the research-result level while the other four ranked rows all won. The mechanism was not a lack of Brighton dominance: it was **high conversion and a state change**. Brighton converted attacking sequences into goals rather than repeated blocks/end-line exits, and Coventry's second-half red card further altered the possession/finishing/corner relationship.

**What was right.** Brighton non-loss, 1H O0.5, Coventry U1.5 and O2.5 all matched the realised score path. The card correctly separated Coventry's scoring drought from pure xG zeroes and correctly expected Brighton to create enough to win.

**Knowability audit.** The possibility that an early lead/high conversion reduces later corner demand was named pregame. The red card was an in-game shock and must not be back-fitted as a pregame miss. The pregame error was assigning too much mass to “territorial dominance → 4+ corners” relative to “territorial dominance → direct goals / low corner conversion”.

**Smallest routine change.** In the existing soccer corner tree, print a distinct **high-conversion / low-corner** branch whenever the same side is also projected for strong goal creation. This is a disclosure/branch-completeness reinforcement, not an automatic Under-corners adjustment.

**Learning disposition.** Strong worked example for `RULES_SOCCER.md` controls that corners are not a dominance proxy and that score state changes later corner demand. Do **not** promote a universal lower-corner weight from one game. Corner grade remains provisional because the frozen Premier League/Opta field-owner record was not directly recovered.

**Settlement/retrospective sources added in this pass**
- https://www.brightonandhovealbion.com/media-article/mft-pl-covbha-match-report-coventry-city-x-albion-x
- https://www.theguardian.com/football/match/2026/sep/13/coventry-v-brightonfootball
- https://www.zerozero.pt/jogo/2026-09-13-coventry-city-brighton-hove-albion/12253122

---

### 0.409 Partial event custody — P-409
...
**Current status:** `FINAL / PARTIAL — derivative Rank(s) 2 remain provisional / NOT BOOKED`  
**Verified sporting result:** Lille 2–0 Troyes; HT 1–0; current match-stat records show corners Lille 2–5 Troyes.  
**Evidence status:** `LEARNING_ONLY / NOT PERFORMANCE_ELIGIBLE`; original issued ranks/probabilities remain frozen.

| Rank | Frozen issued row | p(win) | Result | Brier | Booking state |
|---:|---|---:|---|---:|---|
| 1 | Lille team goals Over 0.5 | 82% | WIN | 0.0324 | WIN |
| 2 | Troyes team corners Over 2.5 | 79% | WIN | — | PROVISIONAL RESEARCH WIN — Troyes 5 corners; exact frozen LFP/defined-provider field not directly recovered |
| 3 | Lille or Draw | 76% | WIN | 0.0576 | WIN |
| 4 | 1H Over 0.5 goals | 70% | WIN | 0.0900 | WIN |
| 5 | Full match Under 3.5 goals | 65% | WIN | 0.1225 | WIN |

**Definitively booked ranked rows on this card:** 4 W / 0 L; mean row-level Brier **0.0756**. This is an audit statistic only; complementary/nested rows are not independent trials.

**Potential winner:** Lille 51% → **WIN**; potential-winner Brier **0.2401**.

**Additional published supplied-line call:** Over 2.5 goals 57% → **LOSS** (2 goals). It is recorded but excluded from the five-row ranked denominator.

**Result driver.** Lille scored early and controlled the result to 2–0. Troyes still generated the corner volume needed for O2.5 despite only ~37% possession in current match-stat records. That is a useful positive example of the card's stated mechanism: a side can accumulate corners through chasing, wide entries and blocked/end-line events without dominating possession.

**What was right.** Lille O0.5, Lille/Draw, 1H O0.5 and U3.5 all won. The additional supplied O2.5 lean (57%, outside the ranked five) **lost** because the match stayed at two goals until full time; the 90th-minute Lille goal moved 1–0 to 2–0, not into the Over.

**Knowability audit.** The card had both the 2-goal and 3+-goal families and deliberately ranked U3.5 above O2.5. No hindsight change is needed. Troyes' corner count is supported by two current displays but is kept provisional because the exact frozen LFP/defined-provider field was not directly recovered.

**Smallest routine change.** None to probability direction. Preserve separate goal and corner processes and keep possession out of the corner model unless linked to actual corner-causing events.

**Learning disposition.** Positive worked example for `RULES_SOCCER.md` controls 4/14/25; no new rule.

**Settlement/retrospective sources added in this pass**
- https://www.soccernews.com/matches/lille-osc-vs-es-troyes-ac-ligue-1-2026-27-559685/
- https://www.statmuse.com/fc/ask/team-game-log-corners-at?l=ligue1
- https://ligue1.com/fr/articles/l1_article_5562-programmation-tv-j4-2

---

### 0.410 Partial event custody — P-410
...
**Current status:** `FINAL / PARTIAL — derivative Rank(s) 5 remain provisional / NOT BOOKED`  
**Verified sporting result:** RB Leipzig 5–0 Hamburger SV; HT 2–0; current match-stat records show Leipzig at least 7 corners.  
**Evidence status:** `LEARNING_ONLY / NOT PERFORMANCE_ELIGIBLE`; original issued ranks/probabilities remain frozen.

| Rank | Frozen issued row | p(win) | Result | Brier | Booking state |
|---:|---|---:|---|---:|---|
| 1 | RB Leipzig team goals Over 0.5 | 88% | WIN | 0.0144 | WIN |
| 2 | RB Leipzig or Draw | 86% | WIN | 0.0196 | WIN |
| 3 | Hamburger SV team goals Under 1.5 | 82% | WIN | 0.0324 | WIN |
| 4 | 1H Over 0.5 goals | 76% | WIN | 0.0576 | WIN |
| 5 | RB Leipzig team corners Over 4.5 | 68% | WIN | — | PROVISIONAL RESEARCH WIN — 7 corners on Guardian/StatMuse/SoccerNews; one secondary feed reports 8, so provider-specific field remains unbooked |

**Definitively booked ranked rows on this card:** 4 W / 0 L; mean row-level Brier **0.0310**. This is an audit statistic only; complementary/nested rows are not independent trials.

**Potential winner:** RB Leipzig 64% → **WIN**; potential-winner Brier **0.1296**.

**Additional published supplied-line call:** Over 2.5 goals 59% → **WIN** (5 goals). It is recorded but excluded from the five-row ranked denominator.

**Result driver.** Leipzig's current shot/xG superiority and Hamburg's very weak attacking start translated into a 5–0 win. The central 2–0/2–1 corridor got the direction right but understated the **blowout/separation tail**: Leipzig scored three times from 72' to 77'.

**What was right.** The first four ranked rows all won cleanly. The corner research result also clears O4.5 on every current source recovered, but provider counts disagree (7 on Guardian/StatMuse/SoccerNews versus 8 on another secondary feed), so the row is not booked to the frozen provider standard.

**Knowability audit.** The card knew Leipzig had a short European turnaround and expected rotation. The mistake was not “failing to know the score”; it was allowing that uncertainty to act mostly as a suppressor while underrepresenting the upside of fresh/rotated attacking depth. The late three-goal cluster is exactly why bench/substitution states belong in the second-half score tree.

**Smallest routine change.** When rotation is expected but the bench remains high quality, carry both lower-cohesion and fresh-depth upside branches. Under `G-L2`, uncertainty alone remains width; it is not an automatic negative sign.

**Learning disposition.** Reinforce `RULES_SOCCER.md` bench/minutes and late-state controls. No fitted rotation coefficient.

**Settlement/retrospective sources added in this pass**
- https://www.bundesliga.com/en/bundesliga/matchday/2026-2027/3/rb-leipzig-vs-hamburger-sv/liveticker
- https://www.theguardian.com/football/match/2026/sep/13/red-bull-leipzig-v-hamburg
- https://www.statmuse.com/fc/ask/rb-leipzig-corners-game-log
- https://www.soccernews.com/matches/rb-leipzig-vs-hamburger-sv-bundesliga-2026-27-565799/

---

### 0.419 Partial event custody — P-419
...
**Current status:** `FINAL / PARTIAL — derivative Rank(s) 5 remain provisional / NOT BOOKED`  
**Verified sporting result:** Djurgården 2–0 GAIS; HT 1–0; current secondary match-stat records show corners 2–2.  
**Evidence status:** `LEARNING_ONLY / NOT PERFORMANCE_ELIGIBLE`; original issued ranks/probabilities remain frozen.

| Rank | Frozen issued row | p(win) | Result | Brier | Booking state |
|---:|---|---:|---|---:|---|
| 1 | Djurgården team goals Over 0.5 | 84% | WIN | 0.0256 | WIN |
| 2 | Djurgården or Draw | 83% | WIN | 0.0289 | WIN |
| 3 | GAIS team goals Under 1.5 | 79% | WIN | 0.0441 | WIN |
| 4 | Under 3.5 total goals | 70% | WIN | 0.0900 | WIN |
| 5 | Total corners Over 7.5 | 69% | LOSS | — | PROVISIONAL RESEARCH LOSS — 2-2 corners on two current secondary match-stat displays; no frozen field owner/provider |

**Definitively booked ranked rows on this card:** 4 W / 0 L; mean row-level Brier **0.0472**. This is an audit statistic only; complementary/nested rows are not independent trials.

**Potential winner:** Djurgården 60% → **WIN**; potential-winner Brier **0.1600**.

**Additional published supplied-line calls:** 1H O0.5 62% → **WIN**; full-game U2.5 51% → **WIN**; O2.5 49% → **LOSS**. These were outside the ranked five and are not included in the ranked-row denominator.

**Result driver.** Djurgården won 2–0 after leading 1–0 at half-time. The first four ranked goal/side rows all won. The corner research result, however, was only **2–2**, so the self-generated O7.5 alternate lost at the research level.

**Why the corner row failed.** The game produced enough scoring to validate the goal model but not enough corner-causing events. This is another direct example that score dominance, possession and total corners are separate processes. Red-card disruption did not mechanically create a high-corner chase.

**Knowability audit.** The card did build a separate corner process, but the self-generated line still carried no exact frozen provider/operator. Two current secondary stat displays agree on 2–2, so the result is a strong research indication but remains unbooked.

**Smallest routine change.** Do not generate a corner alternate unless a reachable settlement provider is frozen first. If the user did not supply the derivative, source reachability should precede offering it.

**Learning disposition.** Reinforce `RULES_SOCCER.md` corner independence and `SOURCES.md`/`DATA_SOURCE_REGISTER.md` derivative-source gate. No automatic Under-corners adjustment.

**Settlement/retrospective sources added in this pass**
- https://ofstats.com/matches/view/djurgarden-gais-2026-09-14
- https://redscores.com/team/gais/1777
- https://allsvenskan.se/nyheter/sa-spelas-omgang-18-23-av-allsvenskan/

---

## 1. FINAL / SETTLED P-407+ EVENTS — WORKING-ID ORDER
...
**Verified sporting result:** Spain 81–58 Germany; total 139; Spain margin +23.  
**Evidence status:** `LEARNING_ONLY / NOT PERFORMANCE_ELIGIBLE`; original issued ranks/probabilities remain frozen.

| Rank | Frozen issued row | p(win) | Result | Brier | Booking state |
|---:|---|---:|---|---:|---|
| 1 | Germany +6.5 | 56% | LOSS | 0.3136 | LOSS |
| 2 | Under 147.5 | 53% | WIN | 0.2209 | WIN |
| 3 | Over 147.5 | 47% | LOSS | 0.2209 | LOSS |
| 4 | Spain -6.5 | 44% | WIN | 0.3136 | WIN |

**Definitively booked ranked rows on this card:** 2 W / 2 L; mean row-level Brier **0.2672**. This is an audit statistic only; complementary/nested rows are not independent trials.

**Potential winner:** Spain 60% → **WIN**; potential-winner Brier **0.1600**.

**Deep Rank-#1 retrospective.** Spain won 81–58, so Germany +6.5 failed by a very large margin while Spain -6.5 won. The total stayed Under 147.5 at 139. The principal error was therefore **margin allocation**, not pace/total.

The card correctly refused to copy Spain's earlier 83–53 win as the centre because the opener contained extreme shooting splits. However, it then gave too much weight to Germany's rebound/ball-security, home-crowd and recovery branches relative to the repeatable parts of Spain's direct matchup advantage. The rematch again produced a large Spanish separation, which indicates that the first game contained more persistent matchup structure than the retrospective decomposition allowed.

**Knowability audit.** Spain's physical/defensive disruption in the first meeting was known. Germany's Belgium win and improved tournament form were also real and should not be erased after the fact. What must improve is the decomposition of a prior blowout into (a) unstable shooting variance and (b) potentially repeatable possession, rim/paint, turnover, matchup and shot-quality mechanisms.

**Smallest routine change.** For same-tournament rematches after a large first result, require a “repeatable-vs-variance” table before shrinking the prior margin. Do not assign a universal rematch coefficient.

**Learning disposition.** Candidate process addition for `RULES_BASKETBALL.md`; candidate testing note for `LEARNING_REGISTER.md`, explicitly `TESTING/CANDIDATE`, not `PROMOTED`. The game also reinforces the existing principle that a lower total can coexist with a large favourite margin.

**Settlement/retrospective sources added in this pass**
- https://www.fiba.basketball/en/news/2026-wwc-game-center-final
...
**Current status:** `FINAL / SETTLED`  
**Verified sporting result:** Pittsburgh Steelers 20–13 Atlanta Falcons; total 33; Pittsburgh margin +7.  
**Evidence status:** `LEARNING_ONLY / NOT PERFORMANCE_ELIGIBLE`; original issued ranks/probabilities remain frozen.

| Rank | Frozen issued row | p(win) | Result | Brier | Booking state |
|---:|---|---:|---|---:|---|
| 1 | Atlanta Falcons +6.5 | 55% | LOSS | 0.3025 | LOSS |
| 2 | Under 40.5 | 53% | WIN | 0.2209 | WIN |
| 3 | Over 40.5 | 47% | LOSS | 0.2209 | LOSS |
| 4 | Pittsburgh Steelers -6.5 | 45% | WIN | 0.3025 | WIN |

**Definitively booked ranked rows on this card:** 2 W / 2 L; mean row-level Brier **0.2617**. This is an audit statistic only; complementary/nested rows are not independent trials.

**Potential winner:** Pittsburgh Steelers 66% → **WIN**; potential-winner Brier **0.1156**.

**Deep Rank-#1 retrospective.** Pittsburgh won 20–13. Atlanta +6.5 lost by **half a point**, while Under 40.5 and Pittsburgh -6.5 won. The decisive separation event was a fourth-quarter T.J. Watt pick-six. That is the exact kind of non-offensive scoring / short-field branch required by `AM-B4`.

**What was right.** The low total and Pittsburgh winner were correct. The pregame card also correctly froze Cooper Rush as Atlanta's starter and did not pretend the quarterback situation was normal.

**Knowability audit.** The exact interception return was not knowable. The elevated turnover/decision-risk environment under a backup quarterback was knowable and should have been more visibly represented in the spread complement. Because the line was only 6.5, one defensive touchdown was enough to move an otherwise close low-total game from Atlanta cover to Pittsburgh cover.

**Smallest routine change.** On low-total NFL spreads around one score, explicitly print the probability mass for a defensive/special-teams TD or turnover-created short field before ranking the underdog cushion.

**Learning disposition.** Reinforces existing `RULES_AMERICAN_FOOTBALL.md` controls 4, 13 and mandatory branch `AM-B4`. No new turnover coefficient from one game.

**Settlement/retrospective sources added in this pass**
- https://www.nfl.com/schedules/2026/by-team/atlanta-falcons
- https://www.nfl.com/schedules/2026/by-team/pittsburgh-steelers
...
**Current status:** `FINAL / SETTLED`  
**Verified sporting result:** Baltimore Ravens 41–23 Indianapolis Colts; total 64; Baltimore margin +18.  
**Evidence status:** `LEARNING_ONLY / NOT PERFORMANCE_ELIGIBLE`; original issued ranks/probabilities remain frozen.

| Rank | Frozen issued row | p(win) | Result | Brier | Booking state |
|---:|---|---:|---|---:|---|
| 1 | Indianapolis Colts +3.0 | 53% | LOSS | 0.2809 | LOSS |
| 2 | Under 48.5 | 52% | LOSS | 0.2704 | LOSS |
| 3 | Over 48.5 | 48% | WIN | 0.2704 | WIN |
| 4 | Baltimore Ravens -3.0 | 39% | WIN | 0.3721 | WIN |

**Definitively booked ranked rows on this card:** 2 W / 2 L; mean row-level Brier **0.2984**. This is an audit statistic only; complementary/nested rows are not independent trials.

**Potential winner:** Baltimore Ravens 56% → **WIN**; potential-winner Brier **0.1936**.

**Deep Rank-#1 retrospective.** Baltimore won 41–23 and the game reached 64 points. Indianapolis +3 and Under 48.5 both lost; Baltimore -3 and Over won. The card's centre near 48 points and close margin badly underrepresented Baltimore's explosive ceiling.

Baltimore generated 506 yards at 7.9 yards per play. Lamar Jackson threw for 324 yards, Derrick Henry ran for 144 and three TDs, and explosive plays including a 54-yard Zay Flowers score and a 32-yard Henry TD rapidly widened the game. This was not merely “Colts defence bad”; it was a coherent explosive/run-game state.

**Knowability audit.** Baltimore's established Jackson/Henry explosive capability was known. New-season coordinator/secondary uncertainty should have widened *both* tails. The pregame mistake was allowing uncertainty to dilute Baltimore's upside rather than preserving a sufficiently large high-efficiency branch.

**Smallest routine change.** For NFL opener/new-regime cards, display the favourite's intact-star upside branch separately from the uncertainty branch; `new regime = width`, not an automatic downgrade.

**Learning disposition.** Negative worked example for `RULES_AMERICAN_FOOTBALL.md` control 16 and `AM-B2`. No league-wide Week-1 adjustment.

**Settlement/retrospective sources added in this pass**
- https://www.nfl.com/schedules/2026/by-team/baltimore-ravens
- https://www.nfl.com/players/lamar-jackson/stats/logs/
- https://www.nfl.com/players/derrick-henry/stats/logs/2026/
...
**Current status:** `FINAL / SETTLED`  
**Verified sporting result:** Buffalo Bills 36–31 Houston Texans; total 67; Buffalo margin +5.  
**Evidence status:** `LEARNING_ONLY / NOT PERFORMANCE_ELIGIBLE`; original issued ranks/probabilities remain frozen.

| Rank | Frozen issued row | p(win) | Result | Brier | Booking state |
|---:|---|---:|---|---:|---|
| 1 | Houston Texans +1.5 | 55% | LOSS | 0.3025 | LOSS |
| 2 | Under 44.5 | 53% | LOSS | 0.2809 | LOSS |
| 3 | Over 44.5 | 47% | WIN | 0.2809 | WIN |
| 4 | Buffalo Bills -1.5 | 45% | WIN | 0.3025 | WIN |

**Definitively booked ranked rows on this card:** 2 W / 2 L; mean row-level Brier **0.2917**. This is an audit statistic only; complementary/nested rows are not independent trials.

**Potential winner:** Buffalo Bills 53% → **WIN**; potential-winner Brier **0.2209**.

**Deep Rank-#1 retrospective.** Buffalo won 36–31. Texans +1.5 and Under 44.5 lost; Bills -1.5 and Over won. The final total exceeded the line by 22.5 points and Buffalo generated 409 yards on only 52 plays, an explosive 7.9 yards per play.

**What failed.** Houston's prior elite defensive baseline and the previous matchup were given too much practical influence relative to Buffalo's current protection/weapons and explosive efficiency. The card formally treated old H2H as context, but the distribution still behaved as though the prior suppression regime would transfer more strongly than it did.

**Knowability audit.** Buffalo's stable offensive infrastructure and Josh Allen's explosive ceiling were knowable; Houston's current defensive quality was also reasonable evidence. This is a transfer/width problem, not proof that defensive priors should be discarded.

**Smallest routine change.** When a prior-season elite defence meets a materially changed offence, print a “prior transfers / partial transfer / fails to transfer” branch table and do not let last-season suppression silently dominate the total.

**Learning disposition.** Candidate worked example for `RULES_AMERICAN_FOOTBALL.md`; no numerical decay rate is justified.

**Settlement/retrospective sources added in this pass**
- https://www.nfl.com/schedules/2026/by-team/houston-texans
- https://www.nfl.com/schedules/2026/by-team/buffalo-bills
...
**Current status:** `FINAL / SETTLED`  
**Verified sporting result:** New York Yankees 2–0 New York Mets; total 2; Yankees margin +2.  
**Evidence status:** `LEARNING_ONLY / NOT PERFORMANCE_ELIGIBLE`; original issued ranks/probabilities remain frozen.

| Rank | Frozen issued row | p(win) | Result | Brier | Booking state |
|---:|---|---:|---|---:|---|
| 1 | Under 8.0 runs | 52% | WIN | 0.2304 | WIN |
| 2 | Mets +1.5 | 51% | LOSS | 0.2601 | LOSS |
| 3 | Yankees -1.5 | 49% | WIN | 0.2601 | WIN |
| 4 | Over 8.0 runs | 37% | LOSS | 0.1369 | LOSS |

**Definitively booked ranked rows on this card:** 2 W / 2 L; mean row-level Brier **0.2219**. This is an audit statistic only; complementary/nested rows are not independent trials.

**Potential winner:** New York Yankees 63% → **WIN**; potential-winner Brier **0.1369**.

**Result driver.** The Yankees won 2–0. Cam Schlittler threw six shutout innings and allowed one hit; both Yankee runs came on solo homers. Under 8 won, Yankees -1.5 won, Mets +1.5 lost and the Yankees winner call won.

**What this teaches.** This is a clean example of the existing baseball rule that **low total does not imply close margin**. A 2–0 game can simultaneously be a strong run-suppression result and a favourite -1.5 cover.

**Knowability audit.** Starter quality and a strong run-prevention path were known. The exact two solo-homer allocation was not. The card's Under probability was only 52% with explicit push mass at 8, so the realised 2-run total should not be used to retroactively claim the Under was much more certain.

**Smallest routine change.** None. Keep margin and total as separate queries of the same run distribution and preserve shutout/2–0/3–0 states.

**Learning disposition.** Positive worked example for `RULES_BASEBALL.md` low-total-separation control.

**Settlement/retrospective sources added in this pass**
- https://www.mlb.com/mets/video/schlittler-rice-and-wells-power-yankees-2-0-win
- https://www.reuters.com/sports/mlb-roundup-yankees-shut-out-mets-move-brink-playoff-berth--flm-2026-09-14/
...
**Current status:** `FINAL / SETTLED`  
**Verified sporting result:** Chunichi Dragons 6–0 Hanshin Tigers; total 6; Chunichi margin +6.  
**Evidence status:** `LEARNING_ONLY / NOT PERFORMANCE_ELIGIBLE`; original issued ranks/probabilities remain frozen.

| Rank | Frozen issued row | p(win) | Result | Brier | Booking state |
|---:|---|---:|---|---:|---|
| 1 | Chunichi Dragons +1.5 | 62% | WIN | 0.1444 | WIN |
| 2 | Under 5.5 runs | 55% | LOSS | 0.3025 | LOSS |
| 3 | Over 5.5 runs | 45% | WIN | 0.3025 | WIN |
| 4 | Hanshin Tigers -1.5 | 38% | LOSS | 0.1444 | LOSS |

**Definitively booked ranked rows on this card:** 2 W / 2 L; mean row-level Brier **0.2235**. This is an audit statistic only; complementary/nested rows are not independent trials.

**Potential winner:** Hanshin Tigers 55% → **LOSS**; potential-winner Brier **0.3025**.

**Result driver.** Chunichi won 6–0. Kyle Muller threw a complete-game four-hit shutout and also drove in **four runs**, including a two-run homer; Hosokawa also homered. Dragons +1.5 and Over 5.5 won, while Hanshin -1.5, Under 5.5 and the Hanshin winner call lost.

**Knowability audit.** The card already gave Chunichi 39% outright-win mass and ranked the +1.5 cushion first, so the upset was not outside the forecast tree. The unusual part was the *way* it happened: a starting pitcher creating four runs with the bat is a rare realised tail. It is not a general reason to add an NPB pitcher-hitting coefficient.

**Smallest routine change.** None to weights. Preserve NPB no-DH/pitcher-batting exposure as a real but usually low-mass component and keep winner probability separate from run-line cushion probability.

**Learning disposition.** Reinforce `RULES_BASEBALL.md` league-specific rules/role exposure; no new predictive rule.

**Settlement/retrospective sources added in this pass**
- https://npb.jp/bis/eng/2026/games/s2026091401939.html
...
**Current status:** `FINAL / SETTLED`  
**Verified sporting result:** Chicago Cubs 7–3 Atlanta Braves; total 10; Cubs margin +4.  
**Evidence status:** `LEARNING_ONLY / NOT PERFORMANCE_ELIGIBLE`; original issued ranks/probabilities remain frozen.

| Rank | Frozen issued row | p(win) | Result | Brier | Booking state |
|---:|---|---:|---|---:|---|
| 1 | Atlanta Braves +1.5 | 57% | LOSS | 0.3249 | LOSS |
| 2 | Over 9.5 runs | 53% | WIN | 0.2209 | WIN |
| 3 | Under 9.5 runs | 47% | LOSS | 0.2209 | LOSS |
| 4 | Chicago Cubs -1.5 | 43% | WIN | 0.3249 | WIN |

**Definitively booked ranked rows on this card:** 2 W / 2 L; mean row-level Brier **0.2729**. This is an audit statistic only; complementary/nested rows are not independent trials.

**Potential winner:** Chicago Cubs 56% → **WIN**; potential-winner Brier **0.1936**.

**Deep Rank-#1 retrospective.** Chicago won 7–3. Braves +1.5 lost; Cubs -1.5 and Over 9.5 won. Reynaldo López allowed five runs in three innings, and Atlanta used A.J. Smith-Shawver for length; Pete Crow-Armstrong later hit a two-run homer off him.

**What failed.** The card identified López's return/short-start uncertainty but did not assign enough mass to the combination “starter fails early + favourite separates before the game reaches higher-leverage bullpen states”. Braves +1.5 was therefore too protected relative to the very mechanism already named.

**Knowability audit.** López's recent return/uncertainty was known. The exact inning cluster was not. The Over cleared by only **0.5 run**, so this result is also threshold-fragile and should not be treated as strong proof of an Over bias.

**Smallest routine change.** In return/limited-starter games, explicitly join the starter-exit distribution to the **score-state relief ladder**: who absorbs length when behind matters as much as bullpen freshness.

**Learning disposition.** Reinforces `RULES_BASEBALL.md` early-hook, score-state bullpen and favourite-separation controls.

**Settlement/retrospective sources added in this pass**
- https://www.reuters.com/sports/baseball/pete-crow-armstrong-blasts-homer-cubs-take-down-braves--flm-2026-09-15/
...
**Current status:** `FINAL / SETTLED`  
**Verified sporting result:** New York Yankees 8–3 Minnesota Twins; total 11; Yankees margin +5.  
**Evidence status:** `LEARNING_ONLY / NOT PERFORMANCE_ELIGIBLE`; original issued ranks/probabilities remain frozen.

| Rank | Frozen issued row | p(win) | Result | Brier | Booking state |
|---:|---|---:|---|---:|---|
| 1 | New York Yankees -1.5 | 52% | WIN | 0.2304 | WIN |
| 2 | Over 8.0 runs | 50% | WIN | 0.2500 | WIN |
| 3 | Minnesota Twins +1.5 | 48% | LOSS | 0.2304 | LOSS |
| 4 | Under 8.0 runs | 38% | LOSS | 0.1444 | LOSS |

**Definitively booked ranked rows on this card:** 2 W / 2 L; mean row-level Brier **0.2138**. This is an audit statistic only; complementary/nested rows are not independent trials.

**Potential winner:** New York Yankees 65% → **WIN**; potential-winner Brier **0.1225**.

**Result driver.** The Yankees won 8–3. They trailed 2–1 after six and the game was tied 2–2 entering the eighth before a **six-run eighth inning**, highlighted by Aaron Judge's three-run homer. Yankees -1.5 and Over 8 won; Twins +1.5 and Under 8 lost.

**Why the call worked.** The final side/total directions were correct, but the mechanism was not an early starter blowout. Separation occurred late through the bullpen/cluster branch. That distinction matters because a correct result does not validate the wrong causal story.

**Knowability audit.** A late bullpen/HR cluster was a standard forecastable branch; the exact six-run inning was not. The favourite run line and Over were strongly coupled through the same realised inning and must not be counted as independent evidence.

**Smallest routine change.** No new weight. Keep score-state relief chains and late cluster probability explicit whenever a favourite run line and Over share the same upper-tail mechanism.

**Learning disposition.** Positive worked example for `RULES_BASEBALL.md` joint separation/cluster logic.

**Settlement/retrospective sources added in this pass**
- https://www.reuters.com/sports/baseball/aaron-judge-homers-yankees-dump-twins-seal-postseason-bid--flm-2026-09-15/
...
**Current status:** `FINAL / SETTLED`  
**Verified sporting result:** Kansas City Chiefs 31–10 Denver Broncos; total 41; Kansas City margin +21.  
**Evidence status:** `LEARNING_ONLY / NOT PERFORMANCE_ELIGIBLE`; original issued ranks/probabilities remain frozen.

| Rank | Frozen issued row | p(win) | Result | Brier | Booking state |
|---:|---|---:|---|---:|---|
| 1 | Denver Broncos +2.5 | 56% | LOSS | 0.3136 | LOSS |
| 2 | Under 42.5 | 53% | WIN | 0.2209 | WIN |
| 3 | Over 42.5 | 47% | LOSS | 0.2209 | LOSS |
| 4 | Kansas City Chiefs -2.5 | 44% | WIN | 0.3136 | WIN |

**Definitively booked ranked rows on this card:** 2 W / 2 L; mean row-level Brier **0.2672**. This is an audit statistic only; complementary/nested rows are not independent trials.

**Potential winner:** Kansas City Chiefs 54% → **WIN**; potential-winner Brier **0.2116**.

**Deep Rank-#1 retrospective.** Kansas City won 31–10 while the total finished **41**, so Denver +2.5 lost but Under 42.5 won. This is an especially important structural result: a large favourite cover occurred **inside an Under**.

Kansas City's separation came from two linked mechanisms the central score underweighted: Kenneth Walker III ran for 173 yards with a 60-yard TD, and Denver's offence was held to 176 total yards. Bo Nix threw for only 131 yards and committed turnovers. The card had a Chiefs upside branch, but it was framed too often as Chiefs separation *plus* an Over. The realised state was instead **Kansas City run dominance + Denver suppression**.

**Knowability audit.** Walker's role and Mahomes' return were known; a 60-yard TD was an explosive-tail event, not knowable. Denver's new playcaller/regime uncertainty was also known and should have widened the offensive floor as well as the ceiling.

**Smallest routine change.** For favourite spreads, explicitly carry a “favourite separation inside low opponent scoring” branch independent of the Over branch.

**Learning disposition.** Strong reinforcement of existing `RULES_AMERICAN_FOOTBALL.md` control 13 (`low total does not imply close spread`) and `AM-B5`; candidate worked example only.

**Settlement/retrospective sources added in this pass**
- https://www.reuters.com/sports/nfl/kenneth-walker-iii-delivers-kc-masterpiece-chiefs-blast-broncos-31-10--flm-2026-09-15/
- https://www.theguardian.com/sport/2026/sep/14/patrick-mahomes-triumphant-on-nfl-return-as-chiefs-beat-up-misfiring-broncos
...
**Current status:** `FINAL / SETTLED`  
**Verified sporting result:** San Diego Padres 8–7 Colorado Rockies; total 15; Padres margin +1.  
**Evidence status:** `LEARNING_ONLY / NOT PERFORMANCE_ELIGIBLE`; original issued ranks/probabilities remain frozen.

| Rank | Frozen issued row | p(win) | Result | Brier | Booking state |
|---:|---|---:|---|---:|---|
| 1 | San Diego Padres -1.5 | 54% | LOSS | 0.2916 | LOSS |
| 2 | Over 11.0 runs | 48% | WIN | 0.2704 | WIN |
| 3 | Colorado Rockies +1.5 | 46% | WIN | 0.2916 | WIN |
| 4 | Under 11.0 runs | 42% | LOSS | 0.1764 | LOSS |

**Definitively booked ranked rows on this card:** 2 W / 2 L; mean row-level Brier **0.2575**. This is an audit statistic only; complementary/nested rows are not independent trials.

**Potential winner:** San Diego Padres 66% → **WIN**; potential-winner Brier **0.1156**.

**Deep Rank-#1 retrospective.** San Diego won 8–7, so the winner call was right but Padres -1.5 lost. Over 11 won and Rockies +1.5 won. San Diego led 6–1 after three innings, yet Colorado cut the deficit to 6–5 by the end of the sixth and kept the game within one.

**What failed.** The card correctly recognised Coors/high-run volatility and the Over tail, but it coupled the Padres offensive edge too strongly to multi-run separation. A high total creates more paths for the trailing home side to re-enter the margin through middle innings, especially when both starter and bullpen run environments are wide.

**Knowability audit.** Coors, Casey Mize's volatility, Tomoyuki Sugano's recent struggles and bullpen exposure were all known. The exact rally sequence was not. The mistake was distribution geometry: “Padres score a lot” is not equivalent to “Padres win by 2+”.

**Smallest routine change.** On high-width run environments, itemise the state “favourite scores enough to win, dog also reaches 6–7 runs, favourite wins by exactly one” before ranking -1.5.

**Learning disposition.** Reinforce `RULES_BASEBALL.md` run-line/total joint-distribution rule and one-run-home-dog branches; no Coors-specific coefficient from one game.

**Settlement/retrospective sources added in this pass**
- https://www.reuters.com/sports/baseball/surging-padres-edge-rockies-extend-winning-streak-eight--flm-2026-09-15/

---

## 2. ADMINISTRATIVE / NO FORECAST
...
### 2026-09-15 closure audit

`P-415` remains **CLOSED — NOT ISSUED / INTERRUPTED**. No picks, probabilities or potential winner were delivered, so no sporting result is imported, no W/L is assigned and no Brier score is created. This is a positive ledger-integrity example: an interrupted research session must not be converted into a hindsight forecast.

**Destination if later documented:** `METHOD.md` / `EXTERNAL_LOGGING_WORKFLOW.md` as a worked example only. No new rule is required because the existing issuance boundary already controls this case.

---

## 3. INHERITED DRIVE FOLLOW-UP QUEUE — READ-ONLY RECONCILIATION

### 3.1 Newly resolvable inherited item

`TMP-OPEN-20260911-03` — `P-364` potential winner **England to win the 3rd Test** is now **FINAL / WIN**. England chased 130 to reach 130/2, with Joe Root 62* and Jordan Cox 59*, winning the Test by eight wickets and the series 3–0.

- PCB official report: https://www.pcb.com.pk/news-detail/england-beat-pakistan-by-eight-wickets-in-third-test.html
- ICC report: https://www.icc-cricket.com/news/england-seal-convincing-wtc-series-sweep-over-pakistan
- Reuters corroboration: https://www.reuters.com/sports/cricket/cricket-root-takes-charge-guiding-england-series-whitewash-over-pakistan-2026-09-12/

**Local disposition:** retire `TMP-OPEN-20260911-03`. Because Drive is read-only here, this file does not mutate the canonical P-364 record or canonical scorecard. The settlement should be appended to `PREDICTION_LOG_COMBINED_3.md` / `GAME_LOG_STATUS_CURRENT.md` in a future authorised reconciliation.

### 3.2 Inherited result/derivative handles still open

| Handle | Parent / row | Frozen item | Current disposition after this recheck |
|---|---|---|---|
| `TMP-OPEN-20260911-01` | `P-368-C02` | Over 7.5 total corners | PROVISIONAL RESEARCH WIN (9); no frozen provider / official current stat field still not recovered |
| `TMP-OPEN-20260911-02` | `P-369-C01` | Under 10.5 total corners | PROVISIONAL RESEARCH LOSS (11); no frozen provider / official current stat field still not recovered |
| `TMP-OPEN-20260909-01` | `P-341-C03` | Over 7.5 total corners | UNSETTLEABLE to frozen standard; required structured lane remains stale/unavailable |
| `TMP-OPEN-20260909-02` | `P-342-C03` | Over 8.5 total corners | PROVISIONAL RESEARCH WIN (16); narrative/event evidence exists, but frozen field-owner numeric record still not recovered |
| `TMP-OPEN-20260909-03` | `P-126` | event identity + corners | IDENTITY_STATE_CONFLICT — unresolved |
...
| `TMP-OPEN-20260909-06` | `P-176-C05` | Amiens/Versailles U10.5 corners | PROVISIONAL WIN (8) |
| `TMP-OPEN-20260909-07` | `P-178-C05` | Cannes/Le Puy U10.5 corners | PROVISIONAL LOSS (16) |
| `TMP-OPEN-20260909-08` | `P-179-C05` | Thionville/Paris 13 U10.5 corners | PROVISIONAL WIN (9) |
| `TMP-OPEN-20260909-09` | `P-233` | Beijing/Lanzhou O8.5 corners | PROVISIONAL WIN (13) |
| `TMP-OPEN-20260909-10` | `P-234-C03` | Dalian/Shenhua O8.5 corners | PROVISIONAL WIN (10; disrupted match) |
| `TMP-OPEN-20260909-11` | `P-235` | Shandong/Shanghai Port O8.5 corners | PROVISIONAL WIN (14) |

Fresh targeted re-probes did **not** justify upgrading these fields to booked W/L status. In particular, secondary corner displays were not allowed to replace a missing frozen field owner.

### 3.3 Historical documentary-audit follow-ups still open

| Handle | Parent | Audit gap | Current disposition |
|---|---|---|---|
| `TMP-AUDIT-20260912-01` | `P-250-C05` | Yunnan Yukun vs Chongqing | later corner adjudication/source reconstruction missing; fresh secondary display shows 1-3 but no frozen field-owner settlement recovered |
| `TMP-AUDIT-20260912-02` | `P-251-C05` | Sassuolo vs Frosinone | later corner adjudication/source reconstruction missing; secondary 6-5 display is insufficient to overwrite audit state |
| `TMP-AUDIT-20260912-03` | `P-255-C05` | Inter Women vs Wolfsburg Women | later corner adjudication/source reconstruction missing |
| `TMP-AUDIT-20260912-04` | `P-256-C05` | PSG Women vs Eintracht Frankfurt Women | later corner adjudication/source reconstruction missing |
| `TMP-AUDIT-20260912-05` | `P-265-C05` | Toluca vs Club León | later corner adjudication/source reconstruction missing; secondary 4-5 display does not satisfy frozen field-owner requirement |

These remain documentary/source-reconstruction tasks rather than new prediction rows. No new canonical IDs are created for them.

---

## 4. COHORT AUDIT — P-407 THROUGH P-423

### 4.1 Descriptive settlement accounting

- Issued working IDs in this mini-log: **17** (`P-407`–`P-423`).
- Actual forecast cards: **16**; `P-415` was interrupted before issue.
- Cards with a verified sporting final and at least one settled row: **15**.
- Fully unresolved sporting event: **P-418**.
- Ranked rows with definitive booked W/L evidence in this pass: **60 = 39 W / 21 L**.
- Additional derivative rows with provisional research direction but **not booked**: **5**.
- If those provisional research directions are shown only for diagnostic context, the observable ranked-row outcomes are **42 W / 23 L across 65 rows**.
- Mean Brier on the **60 definitively booked ranked rows**: **0.2029** versus the trivial 0.2500 binary baseline.
- Research-observed mean Brier including the five unbooked provisional derivative directions would be **0.2062**, shown only as a sensitivity and **not** entered in a canonical scorecard.
- Rank #1 research outcomes: **7 W / 8 L across 15 final cards**. Strict booked Rank #1 fields: **6 W / 7 L across 13**, because P-407 and P-408 Rank #1 corner fields remain provider-provisional.
- Strict booked `both top two won`: **3/12** cards where both top-two fields are definitively booked. Research-direction sensitivity including provisional corner fields: **5/15**.
- Potential-winner calls on the 15 final forecast cards: **14 correct / 1 incorrect**; mean potential-winner Brier **0.1822**.
- `P-418` and `P-415` are excluded from every W/L/Brier denominator.

**Interpretation boundary:** none of the figures above is a calibration, edge, ROI, model-validation or staking claim. Many rows are complementary or nested outcomes on the same underlying target, so row W/L totals are mechanically dependent. The full cohort remains `LEARNING_ONLY / NOT PERFORMANCE_ELIGIBLE`.

### 4.2 Rank-#1 failures are heterogeneous

The eight research-result Rank-#1 misses were **P-408, P-411, P-412, P-413, P-414, P-420, P-422 and P-423**. They should **not** be collapsed into a single “fix the top pick” coefficient:

- `P-408`: soccer corner-process miss despite a 5–0 dominant win.
- `P-411`: basketball margin allocation/rematch-persistence miss; total direction was correct.
- `P-412`: NFL half-point spread miss driven by a defensive touchdown/non-offensive branch.
- `P-413`: NFL explosive favourite ceiling materially under-massed.
- `P-414`: prior defensive suppression failed to transfer against Buffalo's current offence.
- `P-420`: MLB early-starter failure + score-state long-relief branch created separation.
- `P-422`: NFL favourite blowout occurred **inside the Under**, exposing incorrect coupling between spread and total branches.
- `P-423`: MLB high-total environment did not imply favourite -1.5 separation; Colorado repeatedly re-entered the one-run margin.

### 4.3 Strongest cross-event observations

1. **Corners remain their own process.** P-408 is the clearest example: Brighton scored five from heavy attacking control but had only three corners. P-419 similarly produced a 2–0 score with only four total corners. Conversely P-409 Troyes cleared its team-corner threshold with low possession. This reinforces existing soccer controls; it does not justify a universal Under-corners rule.

2. **Spread/margin and total cannot be coupled mechanically.** P-411, P-416, P-422 and P-423 all show different versions of this. A low total can contain a big favourite cover; a high total can contain a one-run favourite win.

3. **Football explosive and non-offensive branches matter at one-score lines.** P-412's pick-six and P-413/P-422 explosive plays show why `AM-B2`/`AM-B4`/`AM-B5` must carry explicit mass rather than live only in prose.

4. **New-regime uncertainty is genuinely two-sided.** P-413's Baltimore upside and P-422's Denver offensive downside both demonstrate why uncertainty should widen the distribution until current evidence establishes a sign. No blanket “new coach/QB = Under” rule follows.

5. **Baseball separation is often a transition problem.** P-420 was driven by an early López failure and length-relief exposure; P-421 by a six-run eighth inning; P-423 by Colorado's middle-inning counter-rally. The relevant object is starter exit → score-state bullpen/length arms → cluster risk, not “bullpen fresh/tired”.

6. **Provider reachability is part of the forecast, not clerical cleanup.** Five local soccer corner rows remain unbooked despite obvious research directions, and P-418 remains completely unresolved. A derivative should not be offered as a normal high-confidence row when the named settlement field is unlikely to be recoverable.

7. **Correct outcomes do not validate the causal story.** P-421 is a positive outcome where the late bullpen cluster, not a starter blowout, produced the run-line/Over result. Retrospectives must compare mechanism against mechanism, not just mark W.

8. **Threshold fragility must stay visible.** P-412 +6.5 lost by 0.5; P-420 O9.5 won by 0.5. Neither should be used as strong evidence for a large probability correction.

### 4.4 Candidate rule/process changes — no automatic promotion

| Candidate | Evidence in this mini-log | Proposed destination | Status |
|---|---|---|---|
| `C-P407-23-SO-CORNER-HIGH-CONVERSION` — explicitly include high-conversion/low-corner state when the same side is projected for strong attacking dominance | P-408; reinforced by P-419 | `RULES_SOCCER.md` worked example / branch disclosure | **REINFORCE EXISTING**, no new coefficient |
| `C-P407-23-BK-REMATCH-DECOMPOSITION` — same-tournament rematch after a blowout requires repeatable-mechanism vs shooting-variance decomposition | P-411 | `RULES_BASKETBALL.md`; `LEARNING_REGISTER.md` as TESTING candidate | **CANDIDATE**, prospective test only |
| `C-P407-23-AF-NONOFFENSIVE-SPREAD` — make defensive/special-teams scoring mass explicit around one-score spreads | P-412 | `RULES_AMERICAN_FOOTBALL.md` worked example | **REINFORCE EXISTING AM-B4** |
| `C-P407-23-AF-FAVOURITE-UNDER-SEPARATION` — favourite can cover comfortably while total stays Under through opponent suppression | P-422 | `RULES_AMERICAN_FOOTBALL.md` worked example | **REINFORCE EXISTING control 13 / AM-B5** |
| `C-P407-23-BB-SCORESTATE-LENGTH` — return/short starter failure must join directly to likely length/middle relief under the resulting score state | P-420 | `RULES_BASEBALL.md` worked example | **REINFORCE EXISTING** |
| `C-P407-23-BB-HIGH-TOTAL-ONE-RUN` — high-width run environment needs explicit favourite-wins-by-one family rather than tying Over to -1.5 | P-423 | `RULES_BASEBALL.md` worked example | **REINFORCE EXISTING** |
| `C-P407-23-SRC-SETTLEMENT-REACHABILITY` — sparse/niche derivative must prove a realistic post-final source lane before issue | P-407/P-408/P-409/P-410/P-418/P-419 | `SOURCES.md`, `DATA_SOURCE_REGISTER.md`, `RULES_GENERAL.md` source-completeness wording | **CANDIDATE OPERATIONAL CONTROL**, no predictive weight |
| Belgian 2026–27 structure correction — 18 clubs, 34-match classic league, no playoffs, bottom two relegated | P-407 pregame audit | `LEAGUE_RULES_SOCCER.md` | **REFERENCE CORRECTION**, not a learning weight |

`L-087` predictive-weighting firewall applies to every row above. No item is promoted merely because it appeared in this cohort.

### 4.5 New / newly useful source lanes observed

| Source lane | What it was useful for | Recommended status |
|---|---|---|
| FIBA World Cup official event/game-report pages | final score, phase, tournament result | High-priority field owner for FIBA settlement |
| NFL official team schedules and player game logs | final result and player mechanism verification | High-priority NFL field-owner lane |
| NPB English official box scores | exact score, innings, pitchers, HR and player batting lines | High-priority NPB field owner |
| Reuters current MLB/NFL match reports | independent mechanism corroboration after official final | Strong secondary narrative, not definition owner |
| Bundesliga official live ticker | exact final and goal timing | High-priority score field owner |
| Brighton official match report | final/result and red-card/game-state narrative | Club field owner for result narrative |
| Guardian match-stat pages / StatMuse / SoccerNews | fast soccer stat corroboration including corners | **Corroboration only unless the card explicitly freezes that provider** |
| RSSSF + Global Sports Archive for Bhutan | schedule/state cross-check and detection that result is still absent | Useful sparse-league state lane; **not sufficient to invent a final** |
| OFStats / RedScores | current match-stat corroboration for niche soccer | Secondary only; not a substitute for a missing frozen field owner |

### 4.6 Where these notes should go if Drive editing is later authorised

- `PREDICTION_LOG_COMBINED_3.md`: canonical import/reconciliation of P-407–P-423 when the canonical sequence reaches them; preserve all issued local IDs as provenance and assign aliases only if a genuine collision exists then.
- `GAME_LOG_STATUS_CURRENT.md`: retire `TMP-OPEN-20260911-03`; retain the remaining inherited handles; add any imported P-407+ unresolved derivative handles only after canonical reconciliation.
- `RULES_SOCCER.md`: worked examples from P-408/P-409/P-419; source-reachability candidate only if approved.
- `LEAGUE_RULES_SOCCER.md`: Belgian 2026–27 competition-format correction identified by P-407.
- `RULES_BASKETBALL.md`: P-411 rematch decomposition candidate.
- `RULES_AMERICAN_FOOTBALL.md`: P-412/P-413/P-414/P-422 worked examples for non-offensive scores, regime width and low-total separation.
- `RULES_BASEBALL.md`: P-416/P-420/P-421/P-423 worked examples for low-total separation, score-state relief and high-total one-run states.
- `SOURCES.md` and `DATA_SOURCE_REGISTER.md`: settlement-source reachability notes and provider-specific derivative limitations.
- `LEARNING_REGISTER.md`: only candidate/testing rows after review; **nothing in this mini-log warrants automatic `PROMOTED_PROCESS` or a fitted coefficient**.
- `METHOD.md` / `EXTERNAL_LOGGING_WORKFLOW.md`: P-415 can be cited as a worked example of correct “interrupted before issue = no forecast” custody.
- **No new MD document is necessary.** Existing governing files are sufficient; this mini-log itself is the audit artifact requested here.

---

## 5. CURRENT WORKING-ID / CUSTODY INDEX

| Working ID | Event | Current status after this pass |
|---|---|---|
| P-407 | Club Brugge vs Royal Antwerp | FINAL / PARTIAL — Rank #1 corner provisional |
| P-408 | Coventry vs Brighton | FINAL / PARTIAL — Rank #1 corner provisional |
| P-409 | Lille vs Troyes | FINAL / PARTIAL — Rank #2 corner provisional |
| P-410 | RB Leipzig vs Hamburg | FINAL / PARTIAL — Rank #5 corner provisional |
| P-411 | Spain W vs Germany W | FINAL / SETTLED |
| P-412 | Falcons @ Steelers | FINAL / SETTLED |
| P-413 | Ravens @ Colts | FINAL / SETTLED |
| P-414 | Bills @ Texans | FINAL / SETTLED |
| P-415 | Angels @ Nationals | CLOSED — NO FORECAST / INTERRUPTED |
| P-416 | Mets @ Yankees | FINAL / SETTLED |
| P-417 | Chunichi @ Hanshin | FINAL / SETTLED |
| P-418 | Drukpa vs RTC | OPEN — EVENT STATE / RESULT NOT VERIFIED |
| P-419 | Djurgården vs GAIS | FINAL / PARTIAL — Rank #5 corner provisional |
| P-420 | Braves @ Cubs | FINAL / SETTLED |
| P-421 | Yankees @ Twins | FINAL / SETTLED |
| P-422 | Broncos @ Chiefs | FINAL / SETTLED |
| P-423 | Padres @ Rockies | FINAL / SETTLED |

**Next unused local working ID remains `P-424`.**  
**Drive canonical next ID remains `P-372` at this read-only snapshot.**  
**No canonical-ID collision was found in this pass.**

---

## 6. SOURCE / METHODOLOGY NOTE

This retrospective used the original mini-log as the immutable prediction record, the current read-only Drive authority (`METHOD.md`, `RULES_GENERAL.md §16.9`, sport rules, `GAME_LOG_STATUS_CURRENT.md`, source policy and current audit state), and fresh post-event research. Result facts were not allowed to rewrite pregame probabilities or ranks. Where a derivative's field owner/provider could not be recovered, the row was kept provisional even when current secondary displays agreed on the apparent result.

---

## 7. 2026-09-16 CANONICAL RECONCILIATION — repository pass

*(Appended; all text above is byte-identical to the raw file.)*

**Raw file:** archived byte-exact at `archive/mini_logs/originals_2026-09-16/PREDICTION_MINI_RUNNING_LOG_P407_P423_FULL_RETROSPECTIVE_2026-09-15.md`.
- Size: 266,045 bytes.
- SHA-256: `f0c77cf8df3b9614422ceb5ad55b684835b3b0fdbe20c6be45fce5765214edc0`.
- Written to Downloads at 2026-09-15 16:46 AEST, 25 minutes after the unsettled log C was archived.

**Canonical authority:** `PREDICTION_LOG_COMBINED_3.md` §"2026-09-15(b)" (settlement) and §"2026-09-16" (this reconciliation, with full evidence). Learning-only. Nothing above is rewritten.

### 7.1 Per-game audit against the canonical settlement

| ID | This log's settlement | Canonical settlement | Verification 2026-09-16 | Disposition |
|---|---|---|---|---|
| P-407 | 3 W / 1 L booked; C01 provisional (9) | Same | ESPN 9–4 corners; 2026-27 Pro League page not found | Agree; C01 open (canonical `TMP-OPEN-20260915-01`) |
| P-408 | C01 provisional LOSS (3) | C01 **booked LOSS** at the Premier League record | Awoniyi red card 53' at 0–2 confirmed; possession 69.4 % and 22 shots (this log: 66 %, 21) | Canonical booking stands; red card added to the canonical retrospective |
| P-409 | C02 provisional (5) | Same | ESPN 5; Ligue 1 live page JS-only | Agree; open (canonical `-02`) |
| P-410 | C05 provisional (7 v 8) | Same (ESPN 8) | Official stats page JS-only; a WebFetch summary's "7–7" is not on the page | Agree; open (canonical `-03`); provider split recorded |
| P-411 | Settled; Rank #1 LOSS | Same | FIBA final (canonical) | Agree; repeatable-v-variance table adopted as basketball control 25 worked example |
| P-412 | Settled; pick-six named | Same W/L | ESPN scoring plays: Watt 35-yard interception-return TD, Q4 14:05 | **Confirmed**; American football control 20 |
| P-413 / P-414 / P-422 | Settled | Same | ESPN box figures confirmed | Agree; American football control 21 (P-422) |
| P-416 / P-417 / P-420 / P-423 | Settled | Same | MLB statsapi and NPB box figures confirmed | Agree; baseball controls 30–31 |
| P-421 | Settled; "tied 2–2 entering the eighth" | Same W/L | MLB line score: Twins led **2–1** entering the 8th | **Narrative corrected** |
| P-418 | Open — RSSSF lists the 14 Sep fixture, unscored | Open — "identity conflict" on a BBS report dated "2026-09-15" | That BBS report (`bbs.bt/244289`) was published 2026-07-17; RSSSF (updated 11 Sep) lists R15 [Sep 14] Drukpa – RTC | **This log was right.** Canonical re-classified `RESULT_NOT_RECOVERED` |
| P-419 | C05 provisional LOSS (2–2) | Same | ESPN 2+2; three red cards (59', 86', 90+10') | Agree; disruption facts added |
| P-415 | Not issued | Same | — | Agree |

**Aggregates recomputed:**
- Booked rows: 60 rows, 39 W / 21 L, mean Brier 0.2029 — **reproduces**. The canonical 61 rows / 0.2088 is the same set plus P-408-C01 at 0.5625.
- Potential winners: mean Brier 0.1822 over 15 cards — **reproduces**.

### 7.2 Temporary IDs — alias map (canonical numbers are never renumbered)

| This log's handle | Row | Canonical handle |
|---|---|---|
| `TMP-OPEN-20260915-01` | P-407-C01 | `TMP-OPEN-20260915-01` (same) |
| `TMP-OPEN-20260915-02` | P-408-C01 | none — settled LOSS at the Premier League record |
| `TMP-OPEN-20260915-03` | P-409-C02 | `TMP-OPEN-20260915-02` |
| `TMP-OPEN-20260915-04` | P-410-C05 | `TMP-OPEN-20260915-03` |
| `TMP-OPEN-20260915-05` | P-418 | `TMP-OPEN-20260915-04` |
| `TMP-OPEN-20260915-06` | P-419-C05 | `TMP-OPEN-20260915-05` |

- §3.1's `TMP-OPEN-20260911-03` (P-364) was already retired canonically on 2026-09-15.
- §3.2's inherited handles are unchanged.
- `TMP-OPEN-20260914-06` (P-406 six-over rows, from log B) was retired on 2026-09-16 at the ESPN cricket API (Edinburgh 68/2).

### 7.3 Disposition of §4.4 candidates

| This log's candidate | Canonical disposition |
|---|---|
| `C-P407-23-SO-CORNER-HIGH-CONVERSION` | Reinforces soccer control 33; kill path added (red card after the favourite leads) |
| `C-P407-23-BK-REMATCH-DECOMPOSITION` | Worked example under basketball control 25 |
| `C-P407-23-AF-NONOFFENSIVE-SPREAD` | **Adopted as American football control 20**, with a 2025 reference rate (0.217 non-offensive TDs a game; ESPN, 272 games) |
| `C-P407-23-AF-FAVOURITE-UNDER-SEPARATION` | **Adopted as American football control 21** |
| `C-P407-23-BB-SCORESTATE-LENGTH` | **Adopted as baseball control 30** (starter-exit transition inning) |
| `C-P407-23-BB-HIGH-TOTAL-ONE-RUN` | Already baseball control 29(b); kill path added |
| `C-P407-23-SRC-SETTLEMENT-REACHABILITY` | **Promoted as `G-L14`** (`RULES_GENERAL.md` §16.11(m)), plus the soccer control 35 route table |
| Belgian 2026–27 format correction | Checked against the official source before `LEAGUE_RULES_SOCCER.md` is changed — see `PREDICTION_LOG_COMBINED_3.md` §"2026-09-16" |

### 7.4 General learnings, rule changes and sources (canonical)

1. **`G-L13` — a model-summarised retrieval is not a record.** A WebFetch summary reported official Bundesliga corner numbers that the page does not contain.
2. **`G-L14` — settlement route printed at issue and consulted at settlement.** P-406 closed in one request.
3. **`G-L15` — over/under geometry.** Preferred side of forced pairs 27 of 45; free low-threshold team and phase rows 27 of 35.
4. **Disruption facts at settlement** (P-408, P-419).
5. **External variants.** Fingerprint every drop location; never renumber canonical handles; verify external narrative facts at a field owner.

**Sources:** see `SOURCES.md` / `DATA_SOURCE_REGISTER.md` §"2026-09-16". This log's lanes are graded there:
- NFL.com player logs — `CANDIDATE`;
- Reuters and club reports — secondary narrative;
- Guardian, StatMuse, SoccerNews, Global Sports Archive, RedScores, OFStats — corroboration only;
- RSSSF — `RESEARCH`.
