...
Specification status: rebuilt and executable on 2026-07-16
...
node scripts/sportsctl.mjs record --input runtime/next_decision_packet.json
```

The template is intentionally not runnable unchanged. Verbatim request text, one exact primary question, and a real creator are required. `new-request` will not invent them.
...
| Machine record | `runtime/store/journal.jsonl`, `runtime/store/head_anchor.json` | Initialized, zero records |
| Human-readable result view | `PREDICTION_RESULTS_LOG_v5.md` | Empty journal-derived view contract |

## Current empirical truth

- Active models: 0.
- Runtime decision records: 0.
