...
Add-Result -Id 'SPEC-FILES-001' -Passed ($missingFiles.Count -eq 0) -Detail $(
    if ($missingFiles.Count -eq 0) { 'All nine canonical controls plus the development model catalog exist.' }
...
Write-Host "Active model count: $($activeRows.Count). Runtime/model validation is not implied by this static result." -ForegroundColor Yellow
exit 0
