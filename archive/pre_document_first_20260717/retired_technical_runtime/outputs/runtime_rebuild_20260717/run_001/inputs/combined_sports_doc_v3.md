...
## 17. Analysis-only live path

`ACTIVE_ANALYSIS_ONLY` is a separate non-prediction path governed by `SPORTS_ANALYSIS_MODEL_REGISTRY_v1.json` and `SPORTS_ACTIVE_ANALYSIS_MODELS_v1.md`. It accepts only an exact supported LIVE winner scope and a fresh, hashed observation from a model-approved official source. It returns `TOSSUP` or a named-side qualitative lean with ordinal strength.

This path creates no candidate universe, probability distribution, calibration evidence, price evaluation, execution claim or prediction-journal entry. Any request for probability, confidence percentage, odds, edge, EV, stake or profitability must use the quantitative workflow and currently fails closed because that release remains suspended.
