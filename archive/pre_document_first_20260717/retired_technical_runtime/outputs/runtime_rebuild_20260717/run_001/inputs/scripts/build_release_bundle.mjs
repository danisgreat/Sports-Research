...
function isOperative(relativePath) {
  ...
  return /^(?:package(?:-lock)?\.json|README\.md|SPORTS_RESEARCH_AUTHORITY_MANIFEST\.md|SPORTS_RELEASE_MANIFEST_v1\.json|SPORTS_ANALYSIS_MODEL_REGISTRY_v1\.json|combined_sports_doc_v3\.md|SPORTS_(?:DATA_DICTIONARY|MODEL_VALIDATION_AND_SIMULATION_FRAMEWORK|SOURCE_REGISTRY|SCORING_SPECIFICATION|ACCEPTANCE_TESTS)_v3\.md|SPORTS_(?:MODEL_COVERAGE_REGISTRY_v3|MODEL_REGISTRY_v1|REGISTERED_SOURCES_v1|CALIBRATION_LEDGER_v3|SETTLEMENTS_v3)\.csv|SPORTS_(?:DEVELOPMENT_MODEL_CATALOG|ACTIVE_ANALYSIS_MODELS)_v1\.md|NEXT_TIME_MODEL_DEVELOPMENT_PLAYBOOK_v1\.md|RESEARCH_RETROSPECTIVE_2026-07-16\.md|PRE_RUNTIME_PASS_MIGRATION_AUDIT_20260716\.csv|PREDICTION_RESULTS_LOG_v5\.md)$/u.test(relativePath);
}
...
function npmInvocation(args) {
  ...
  // Node 24 on Windows can reject direct .cmd spawning with EINVAL. Invoke the
  // trusted npm shim through ComSpec while keeping every argument hard-coded by
  // this release builder (no user-provided shell text reaches this function).
  return {
...
const commands = {
  node_tests: run(npmTest.command, npmTest.args, resolve(output, 'node_tests.txt')),
...
const manifest = {
  ...
  environment: {
    ...
    npm: version(npmVersion.command, npmVersion.args),
