...
## 8. Analysis-only acceptance

The separate `schema/analysis_acceptance_traceability_v1.csv` contains 16 requirements for the five `ACTIVE_ANALYSIS_ONLY` live-state models. All 16 are `COVERED` by executable tests. They cover exact registry membership and hashes, each sport path, TOSSUP behavior, closed schemas, source freshness/host/scope, competition isolation, standard cricket targets, time-state boundaries, model expiry and source-retest expiry.

Passing this smaller matrix authorizes only the bounded qualitative tier. It does not satisfy, reduce or waive any row in the 88-requirement quantitative acceptance matrix.
