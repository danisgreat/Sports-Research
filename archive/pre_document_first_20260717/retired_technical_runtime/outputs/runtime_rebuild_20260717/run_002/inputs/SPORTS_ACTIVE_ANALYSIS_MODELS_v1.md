...
`ACTIVE_ANALYSIS_ONLY` authorizes a deterministic qualitative live lean from a fresh, hashed, registered official-source snapshot. It does not authorize a probability, confidence percentage, fair price, betting edge, expected value, stake, profitability claim, calibration claim or calibration-eligible quantitative prediction entry. For complete history, its frozen input and output are appended as `ANALYSIS_OUTPUT` records and rendered into the human-readable prediction log.
...
The shipped templates contain sentinels and are not valid evidence until every value and hash is replaced. The CLI prints and optionally writes a deterministic, hashed analysis output; by default it also records the input/output and regenerates `PREDICTION_RESULTS_LOG_v5.md`. Use `--no-record` only for explicit testing or draft work.
