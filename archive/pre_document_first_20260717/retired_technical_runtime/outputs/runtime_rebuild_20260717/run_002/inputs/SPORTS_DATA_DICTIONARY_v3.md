...
## 13. Analysis-only records

The qualitative tier uses two separate closed records and never reuses a quantitative prediction record:

- **analysis request:** analysis ID, exact sport/competition/LIVE/winner scope, exactly two participants, analysis cutoff, one or more fresh hashed official-source observations, and a sport-specific live-state object;
- **analysis output:** exact model ID/version, `ACTIVE_ANALYSIS_ONLY`, source/input/code hashes, `SIDE` or `TOSSUP`, ordinal strength, controlled reason codes, limitations and explicit `numeric_forecast_authorized=false`.

The frozen request and output are stored together in an append-only `ANALYSIS_OUTPUT` journal record. The journal enforces a unique analysis ID, input/output identity, input hash, source-observation hash and strict UTC timestamp. This history is visible in the generated prediction log but remains ineligible for quantitative calibration or profitability reporting.

## 14. User-facing publication and generated log

A ranked card delivered to the user has a separate structured `PUBLICATION` record so the exact chat ranking does not disappear inside prose. It references exactly one prior decision snapshot or analysis ID and stores:

- unique publication ID and strict UTC publication time;
- headline and creator;
- ordered selections with unique integer rank, exact selection/line, verdict and concise rationale;
- optional potential winner; and
- a faithful public summary.

One publication is permitted per frozen target. It cannot create or alter a quantitative prediction, bypass a PASS/WATCH decision, or supply calibration evidence. `PREDICTION_RESULTS_LOG_v5.md` is regenerated from the verified journal and joins publications to their source decision or analysis. Missing historical publications are labelled as not captured; they are never reconstructed from memory.

The schemas are `schema/analysis_request.schema.json` and `schema/analysis_output.schema.json`. These records are not `ELIGIBLE_V3` predictions and must not enter calibration, accuracy, yield or profitability denominators.
