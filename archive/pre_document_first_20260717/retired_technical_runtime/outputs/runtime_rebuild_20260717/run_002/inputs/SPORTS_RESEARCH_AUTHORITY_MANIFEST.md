...
Analysis-only status: **ACTIVE_ANALYSIS_ONLY — FIVE QUALITATIVE LIVE-STATE MODELS**
...
| 3 | `SPORTS_MODEL_COVERAGE_REGISTRY_v3.csv`, `SPORTS_ANALYSIS_MODEL_REGISTRY_v1.json` | quantitative operational allowlist and separately bounded qualitative analysis-only allowlist |
...
| 13 | `runtime/store/journal.jsonl`, `runtime/store/head_anchor.json` | append-only decision/analysis/publication/settlement/evaluation record and local head anchor |
...
- Five models in `SPORTS_ANALYSIS_MODEL_REGISTRY_v1.json` are `ACTIVE_ANALYSIS_ONLY`. They may return a fresh-source qualitative live lean but cannot publish a probability or satisfy quantitative activation.
...
- Runtime journal is append-only and its current counts are established by `sportsctl verify-store`, not by a hard-coded prose total.
...
The word ACTIVE without the complete suffix is reserved for quantitative activation. `ACTIVE_ANALYSIS_ONLY` is a different tier: deterministic, uncalibrated, LIVE-only, and prohibited from publishing probability, confidence percentage, odds, edge, EV, stake or profitability.
...
`SPORTS_ACTIVE_ANALYSIS_MODELS_v1.md` specifies the separately authorized qualitative tier. Its deterministic leans are not predictions eligible for calibration or profitability reporting. They are nevertheless appended as typed `ANALYSIS_OUTPUT` history records so every delivered lean appears in the generated prediction log.
