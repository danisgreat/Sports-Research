...
import { appendAnalysisOutput, appendDecisionPacket, appendEvaluation, appendPublication, appendSettlement, DEFAULT_JOURNAL, initializeJournal, recoverJournalAnchor, verifyJournal } from '../src/journal.mjs';
...
import { DEFAULT_PREDICTION_LOG, renderPredictionLog } from '../src/prediction-log.mjs';
...
function validateSystem() {
  const required = [
    ...
    'src/prediction-log.mjs', 'PREDICTION_RESULTS_LOG_v5.md', 'combined_sports_doc_v3.md', 'templates/publication.template.json',
...
function help() {
  ...
  process.stdout.write(`  analyse-live --input FILE [--output FILE] [--no-record] [--journal FILE] [--log FILE]\n`);
  process.stdout.write(`  init-store [--journal FILE]\n`);
  process.stdout.write(`  new-request --input FILE --output FILE [--publication FILE] [--no-record] [--journal FILE] [--log FILE]\n`);
  process.stdout.write(`  validate-packet --input FILE\n`);
  process.stdout.write(`  record --input FILE [--journal FILE]\n`);
  process.stdout.write(`  publish --input FILE [--journal FILE] [--log FILE]\n`);
  ...
  process.stdout.write(`  render-log [--journal FILE] [--output FILE]\n`);
...
async function main() {
  ...
  if (command === 'analyse-live') {
    ...
    const output = analyseLive(input, { root: ROOT });
    if (options.output) writeJsonAtomic(absolute(options.output), output);
    if (!options['no-record']) {
      const journal = absolute(options.journal, DEFAULT_JOURNAL);
      initializeJournal(journal);
      appendAnalysisOutput(journal, input, output);
      renderPredictionLog(journal, absolute(options.log, DEFAULT_PREDICTION_LOG));
    }
  ...
  if (command === 'render-log') {
    const report = renderPredictionLog(
      absolute(options.journal, DEFAULT_JOURNAL),
      absolute(options.output, DEFAULT_PREDICTION_LOG),
    );
    process.stdout.write(`${JSON.stringify(report, null, 2)}\n`);
    return;
  }
  ...
  if (command === 'new-request') {
    ...
    if (!options['no-record']) {
      ...
      if (options.publication) appendPublication(journal, readJson(absolute(options.publication)));
      renderPredictionLog(journal, absolute(options.log, DEFAULT_PREDICTION_LOG));
    }
    return;
  }
  ...
  if (command === 'publish') {
    if (!options.input) throw new Error('publish requires --input');
    const journal = absolute(options.journal, DEFAULT_JOURNAL);
    initializeJournal(journal);
    const result = appendPublication(journal, readJson(absolute(options.input)));
    renderPredictionLog(journal, absolute(options.log, DEFAULT_PREDICTION_LOG));
    process.stdout.write(`recorded publication sequence=${result.record.sequence}; hash=${result.record.record_hash}\n`);
    return;
  }
