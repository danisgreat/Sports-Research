...
$combinedManual = Read-Text -RelativePath 'combined_sports_doc_v3.md'
$combinedTokens = @(
    'Mandatory request-time runbook',
    'Prediction-log and copy/paste contract',
    'Retrospective learning and settlement',
    'Sport-specific operating instructions',
    'Active qualitative live models',
    'Transfer and readiness checklist'
)
Add-Result -Id 'SPEC-COMBINED-MANUAL-001' -Passed (Test-ContainsAll -Text $combinedManual -Tokens $combinedTokens) -Detail 'Portable combined manual contains general, logging, retrospective, sport-specific and transfer instructions.'

$predictionLog = Read-Text -RelativePath 'PREDICTION_RESULTS_LOG_v5.md'
$predictionLogTokens = @('GENERATED, COPY-FRIENDLY VIEW','Current verified state','Copy-and-paste entries','Journal head')
Add-Result -Id 'SPEC-PREDICTION-LOG-001' -Passed (Test-ContainsAll -Text $predictionLog -Tokens $predictionLogTokens) -Detail 'Generated prediction log exposes verified copy-friendly history.'
