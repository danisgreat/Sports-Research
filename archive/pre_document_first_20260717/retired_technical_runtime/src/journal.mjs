...
const RECORD_TYPES = new Set(['DECISION_PACKET', 'ANALYSIS_OUTPUT', 'PUBLICATION', 'SETTLEMENT', 'EVALUATION']);
...
const PUBLICATION_FIELDS = new Set([
  'publication_id', 'published_at_utc', 'snapshot_id', 'analysis_id', 'headline', 'ranked_selections',
  'potential_winner', 'public_summary', 'created_by',
]);
...
export function verifyJournal(path = DEFAULT_JOURNAL, { checkAnchor = true } = {}) {
  ...
  records.forEach((record, index) => {
    ...
    } else if (record.record_type === 'ANALYSIS_OUTPUT') {
      const input = record.payload?.input;
      const output = record.payload?.output;
      const analysisId = output?.analysis_id;
      if (!analysisId || input?.analysis_id !== analysisId) errors.push(`line ${line}: analysis input/output identity mismatch`);
      else if (analysisIds.has(analysisId)) errors.push(`line ${line}: duplicate analysis_id ${analysisId}`);
      analysisIds.add(analysisId);
      if (output?.schema_version !== 'SPORTS_ANALYSIS_OUTPUT_V1'
        || output?.analysis_tier !== 'ACTIVE_ANALYSIS_ONLY'
        || output?.numeric_forecast_authorized !== false) errors.push(`line ${line}: invalid analysis output envelope`);
      if (output?.input_hash !== hashObject(input)) errors.push(`line ${line}: analysis input hash mismatch`);
      if (output?.source_snapshot_hash !== hashObject(input?.source_observations)) errors.push(`line ${line}: analysis source snapshot hash mismatch`);
      try { parseUtc(output?.as_of_utc, `line ${line}.payload.output.as_of_utc`); } catch (error) { errors.push(error.message); }
    } else if (record.record_type === 'PUBLICATION') {
      const publication = record.payload;
      const targetFields = [publication?.snapshot_id, publication?.analysis_id].filter(Boolean);
      const target = publication?.snapshot_id ? `SNAPSHOT:${publication.snapshot_id}` : `ANALYSIS:${publication?.analysis_id}`;
      if (!publication?.publication_id) errors.push(`line ${line}: publication_id is missing`);
      else if (publicationIds.has(publication.publication_id)) errors.push(`line ${line}: duplicate publication_id ${publication.publication_id}`);
      publicationIds.add(publication?.publication_id);
      if (targetFields.length !== 1) errors.push(`line ${line}: publication must reference exactly one snapshot_id or analysis_id`);
      else if (publication.snapshot_id && !snapshotIds.has(publication.snapshot_id)) errors.push(`line ${line}: publication references unknown prior snapshot`);
      else if (publication.analysis_id && !analysisIds.has(publication.analysis_id)) errors.push(`line ${line}: publication references unknown prior analysis`);
      if (publicationTargets.has(target)) errors.push(`line ${line}: duplicate publication target ${target}`);
      publicationTargets.add(target);
      if (!Array.isArray(publication?.ranked_selections) || publication.ranked_selections.length === 0) errors.push(`line ${line}: publication ranked_selections is empty`);
      try { parseUtc(publication?.published_at_utc, `line ${line}.payload.published_at_utc`); } catch (error) { errors.push(error.message); }
  ...
  return {
    ...
    analysis_count: records.filter((record) => record.record_type === 'ANALYSIS_OUTPUT').length,
    publication_count: records.filter((record) => record.record_type === 'PUBLICATION').length,
...
export function appendAnalysisOutput(path, input, output) {
  return appendJournalRecord(path, 'ANALYSIS_OUTPUT', null, {
    semanticPreflight(existing) {
      if (!input || !output || output.analysis_id !== input.analysis_id) throw new Error('analysis input/output identity mismatch');
      if (output.schema_version !== 'SPORTS_ANALYSIS_OUTPUT_V1'
        || output.analysis_tier !== 'ACTIVE_ANALYSIS_ONLY'
        || output.numeric_forecast_authorized !== false) throw new Error('invalid analysis output envelope');
      if (output.input_hash !== hashObject(input)) throw new Error('analysis input hash mismatch');
      if (output.source_snapshot_hash !== hashObject(input.source_observations)) throw new Error('analysis source snapshot hash mismatch');
      parseUtc(output.as_of_utc, 'analysis output as_of_utc');
      if (existing.some((record) => record.record_type === 'ANALYSIS_OUTPUT'
        && record.payload.output.analysis_id === output.analysis_id)) throw new Error(`duplicate analysis_id ${output.analysis_id}`);
      return { input, output };
    },
  });
}

export function appendPublication(path, publication) {
  return appendJournalRecord(path, 'PUBLICATION', publication, {
    semanticPreflight(existing) {
      const extra = Object.keys(publication).filter((field) => !PUBLICATION_FIELDS.has(field));
      if (extra.length) throw new Error(`publication contains unknown field(s): ${extra.join(', ')}`);
      const required = ['publication_id', 'published_at_utc', 'headline', 'ranked_selections', 'public_summary', 'created_by'];
      const missing = required.filter((field) => publication[field] === null || publication[field] === undefined || publication[field] === '');
      if (missing.length) throw new Error(`publication missing: ${missing.join(', ')}`);
      if ([publication.snapshot_id, publication.analysis_id].filter(Boolean).length !== 1) {
        throw new Error('publication must reference exactly one snapshot_id or analysis_id');
      }
      parseUtc(publication.published_at_utc, 'publication published_at_utc');
      if (!Array.isArray(publication.ranked_selections) || publication.ranked_selections.length === 0) {
        throw new Error('publication ranked_selections must be a nonempty array');
      }
      const ranks = publication.ranked_selections.map((item) => item?.rank);
      if (publication.ranked_selections.some((item) => !Number.isInteger(item?.rank) || item.rank < 1
        || typeof item.selection !== 'string' || !item.selection.trim()
        || typeof item.verdict !== 'string' || !item.verdict.trim()
        || typeof item.rationale !== 'string' || !item.rationale.trim())
        || new Set(ranks).size !== ranks.length) throw new Error('publication ranked_selections are invalid');
      const snapshots = new Set(existing.filter((record) => record.record_type === 'DECISION_PACKET')
        .map((record) => record.payload.packet.decision_snapshot.snapshot_id));
      const analyses = new Set(existing.filter((record) => record.record_type === 'ANALYSIS_OUTPUT')
        .map((record) => record.payload.output.analysis_id));
      if (publication.snapshot_id && !snapshots.has(publication.snapshot_id)) throw new Error('publication references unknown prior snapshot');
      if (publication.analysis_id && !analyses.has(publication.analysis_id)) throw new Error('publication references unknown prior analysis');
      if (existing.some((record) => record.record_type === 'PUBLICATION'
        && record.payload.publication_id === publication.publication_id)) throw new Error(`duplicate publication_id ${publication.publication_id}`);
      if (existing.some((record) => record.record_type === 'PUBLICATION'
        && ((publication.snapshot_id && record.payload.snapshot_id === publication.snapshot_id)
          || (publication.analysis_id && record.payload.analysis_id === publication.analysis_id)))) {
        throw new Error('publication target already has a recorded user-facing output');
      }
      return publication;
    },
  });
