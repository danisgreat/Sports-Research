...
> **Current cross-sport authority — MDS-2026.09.19-v4.3 / CR-2026.09.19-4:** every American-football event requires at least **3 independent reliable source lineages**, timezone-aware venue-local → `Australia/Melbourne` schedule verification with correct AEST/AEDT/date rollover, and three-source explicit terminal-state/result confirmation before settlement. Search snippets/generated summaries do not count; any credible live-state conflict fails closed. This prospective override does not rewrite historical issued cards or this file's v4.0 base provenance.
...
| `G20.2` aggregate tail budget (**disclosure only, per `RULES_GENERAL.md` §15 — the 2026-09-06 v3.8 hard ordinal bars on `G20.2`/`G21.1`/`G26.1` were withdrawn the same day and reclassified as `CANDIDATE`, pending their own prospective tests**) | Second-highest L10 points-per-drive × opponent median drives allowed (with "drive" defined consistently for both sides), both directions, plus a separate special-teams/defensive-score term. |
| `G21.1` total-row path geometry (**disclosure only, per `RULES_GENERAL.md` §15 — the 2026-09-06 v3.8 hard ordinal bars on `G20.2`/`G21.1`/`G26.1` were withdrawn the same day and reclassified as `CANDIDATE`, pending their own prospective tests**) | A first-half `Over 0.5` is a `TRUE_UNION` (any one score wins it); a full-game `Under` is `INTERSECTION_CONSTRAINT` plus the overtime branch. |
