...
> **Current cross-sport authority — MDS-2026.09.19-v4.3 / CR-2026.09.19-4:** every cricket event requires at least **3 independent reliable source lineages**, timezone-aware venue-local → `Australia/Melbourne` schedule verification with correct AEST/AEDT/date rollover, and three-source explicit terminal-state/result confirmation before settlement. Search snippets/generated summaries do not count; any credible live-state conflict fails closed. This prospective override does not rewrite historical issued cards or this file's v4.0 base provenance.
...
## 2. Toss-window pitch report and conditions hard gate

For every cricket card, actively search for **the pitch report** — the exact-match strip/surface assessment — at toss time and immediately before delivery where practical. "Pitch report" and "STRIP STATUS" below name the same requirement; a card that has not produced or explicitly failed the search below has not satisfied this gate, regardless of how much other research it contains.

Store two independent statuses:

- **STRIP STATUS (pitch report):** OBSERVED / NOT FOUND AFTER SEARCH / CONFLICTING;
- **MATCH CONDITIONS STATUS:** OBSERVED / NOT AVAILABLE / CONFLICTING.

**A bare status word without a shown search is not compliant.** Every card must show its work by reproducing the rung-by-rung table from DATA_SOURCE_REGISTER.md §6A: for each of the six rungs, state whether it was attempted, the exact source(s) queried, and the result. `NOT FOUND AFTER SEARCH` is a legitimate, honest outcome for a sparse, associate, or minor domestic fixture where no such report was ever published — but only after the rungs were actually queried and the attempt is shown; it is not a legitimate default for a major international or marquee franchise-league fixture where pitch commentary is routinely published, nor a label to reach for after checking only one or two rungs.

Minimum queries before STRIP STATUS may be marked NOT FOUND AFTER SEARCH:

1. the exact competition's official/broadcast toss commentary or match-centre for a spoken or written pitch assessment;
2. `"<venue name>" pitch report` and `"<venue name>" curator` (or groundsman/groundstaff) — this surfaces both toss-day items and dated venue-profile pieces from reputable cricket journalism; a profile piece not matched to today's date is still usable as a labelled **historical venue tendency**, not this match's confirmed preparation, and must be dated and captioned as such;
3. the competition's specialist match-preview/"pitch and conditions" article (for example an ESPNcricinfo match preview, fetched through the access ladder in RULES_GENERAL.md §4 when the direct route is blocked) for major/international and marquee franchise-league fixtures;
4. for an internationally accredited venue, whether an ICC Pitch and Outfield Monitoring rating exists for recent matches there (`SRC-CRIC-ICC-PITCH-RATING`, DATA_SOURCE_REGISTER.md §6) as multi-year venue-reputation context — retrospective and never a live substitute for today's strip.

Generic fantasy-cricket/tipping "pitch report" pages (the excluded-domain list in DATA_SOURCE_REGISTER.md §6A) are never used, cited, or paraphrased as if they were a field-owner or reporting source.
...
Weather or an automated conditions field cannot imply unreported grass, hardness, pace, seam, turn, or deterioration. If no strip report exists after the shown search, continue with a disclosed evidence limit and forced/low-confidence ranking; never invent the strip.

**Toss decision as weak circumstantial context.** Once the toss result and decision are known, the winning captain's choice to bat or bowl first is a fact, not an invented one, and it is information: the two captains and their support staff typically have direct, informal pitch knowledge the model cannot otherwise access. An unusual decision relative to the format/venue's ordinary norm (for example, electing to bowl first at a venue/format where batting first is normally preferred) may be recorded as `D-conditional` circumstantial context for the conditions branch. It never by itself creates or reverses a directional lean (RULES_CRICKET.md §10.6 override 3 still controls: toss direction alone supports neither an Under nor a winner without a mechanism), and it is always subordinate to an actual observed strip report when one exists.
...
10. Environment block complete per §10.9: the full six-rung attempt-and-result table from DATA_SOURCE_REGISTER.md §6A shown on the card (not only a bottom-line STRIP STATUS word), **at least two independent conditions signals** from different rungs, and the rung-6 venue/format scoring baseline computed and stated with its sample size, split by innings order.
...
|---|---|
| Conditions ladder | Rung-by-rung attempt-and-result shown per §2, **at least two independent signals** from different rungs of DATA_SOURCE_REGISTER.md §6A, and rung 6 always computed |
...
**Anchoring (G12.1).** Anchor every innings and phase total on the venue-and-format distribution from rung 6, split by innings order, with the sample size stated. A chase-innings line and a first-innings line at the same number are different propositions.
...
| `G20.2` aggregate tail budget (**disclosure only, per `RULES_GENERAL.md` §15 — the 2026-09-06 v3.8 hard ordinal bars on `G20.2`/`G21.1`/`G26.1` were withdrawn the same day and reclassified as `CANDIDATE`, pending their own prospective tests**) | For an innings total: each side's second-highest innings score in its own L10 at this format, plus the opponent's median concession, printed as a labelled stress scenario rather than a claimed joint observation. For a phase total: the second-highest and second-lowest phase scores in the retrieved window — the actual observed range, not asserted as statistical "modes" unless the sample genuinely supports that claim. |
| `G21.1` total-row path geometry (**disclosure only, per `RULES_GENERAL.md` §15 — the 2026-09-06 v3.8 hard ordinal bars on `G20.2`/`G21.1`/`G26.1` were withdrawn the same day and reclassified as `CANDIDATE`, pending their own prospective tests**) | A powerplay/phase `Over` set below the phase median is `LOW_BAR_CUMULATIVE` (runs must still accumulate ball by ball; it is not a `TRUE_UNION`, corrected 2026-09-06(d)). A full-innings `Under` is `INTERSECTION_CONSTRAINT` across all 20 overs. A phase line at the median is `CENTRAL_BAND`. |
| `G26.1` top-slot separation floor | Print the row's own reference split. A row inside 40–60% takes Rank #1 only with named event-specific evidence moving it outside the band. |
...
**Pitch/conditions ladder — two rungs added.** The six-rung ladder in `DATA_SOURCE_REGISTER.md` §6A stands. Add, before declaring `NOT FOUND AFTER SEARCH`:

- **Rung 7 — the immediately preceding match at the same venue in the same series/tournament**, with its innings totals and phase splits, recorded **as a different strip** unless a curator or broadcast statement says the same pitch is being reused. This is context for the ground's character, never a substitute for today's report, and `P-457` shows the cost of treating it as one.
- **Rung 8 — the toss broadcast itself**, when the freeze can be placed after it. The toss interview routinely contains the captains' own strip read. Where the contract's activation depends on the toss (`control 32`), a freeze placed after the toss converts two unknowns — activation and strip — into knowns at once. Record explicitly whether the freeze was pre- or post-toss.
