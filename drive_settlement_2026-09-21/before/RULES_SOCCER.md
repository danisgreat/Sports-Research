...
> **Current cross-sport authority — MDS-2026.09.19-v4.3 / CR-2026.09.19-4:** every soccer event requires at least **3 independent reliable source lineages**, timezone-aware venue-local → `Australia/Melbourne` schedule verification with correct AEST/AEDT/date rollover, and three-source explicit terminal-state/result confirmation before settlement. Search snippets/generated summaries do not count; any credible live-state conflict fails closed. This prospective override does not rewrite historical issued cards or this file's v4.0 base provenance.
...
| `G20.2` aggregate tail budget (**disclosure only, per `RULES_GENERAL.md` §15 — the 2026-09-06 v3.8 hard ordinal bars on `G20.2`/`G21.1`/`G26.1` were withdrawn the same day and reclassified as `CANDIDATE`, pending their own prospective tests**) | `T_hi_fav` = favourite's second-highest goals-for in its L10 + opponent's median goals-for; repeat symmetrically. **Correction, 2026-09-06(d):** the post-60' substitution window is **not** an additional term to add on top of the full-match total — it is already included inside it. Partition non-overlapping windows (0-60' and 60-90'+) if a bench-driven late tail needs its own disclosure; never sum a sub-interval total with the full-match total that already contains it. |
| `G21.1` total-row path geometry (**disclosure only, per `RULES_GENERAL.md` §15 — the 2026-09-06 v3.8 hard ordinal bars on `G20.2`/`G21.1`/`G26.1` were withdrawn the same day and reclassified as `CANDIDATE`, pending their own prospective tests**) | `1H Over 0.5` is a `TRUE_UNION` with `N` = enumerated first-half scoring chances (any one goal wins it). `FT Under 2.5` is `INTERSECTION_CONSTRAINT`. A total at the competition median is `CENTRAL_BAND`. |
