...
`tools/sport_models.py` carries an A0 baseline and an A1 challenger for every sport (the list is in [`research/sport_shadow/README.md`](../sport_shadow/README.md)). This folder records how the A1 models did against A0 on every public results set that could be reached from the implementing session, and against TB-1 (`tools/team_baseline.py`) where TB-1 exists.

## Protocol

- **Priors declared first.** Every constant in `tools/sport_models.py` was committed (ac6fdc5) before the first run on real data. Nothing was tuned on these results, with two exceptions, both disclosed below (MLB and tennis v2).
- **Rolling origin, leak-free.** Each game is forecast from games strictly before its date. Ratings are refit on each date, warm-started. Widths come only from earlier out-of-sample residuals. Tennis runs in tournament and round order.
- **Proper scores only.** Each metric is a loss, so lower is better:
  - Brier and log loss on the result;
  - three-way RPS where draws exist;
  - integer RPS (discrete CRPS) on the margin and the total;
  - Brier at one total line, fixed from the pre-window history as floor(mean) + 0.5;
...
- **Market-blind.** No odds, lines or prices are inputs. The NFL file also contains betting columns, and 538's NBA file contains 538's forecasts. The loaders keep only the named score columns, and the other columns are never read.
...
| NBA (2012-13 to 2014-15, the latest public NBA results reachable; 3,689 games) | **A1 better** (−0.036) | **A1 better** | **A1 better** | **Better on both** (−0.009 result, −0.005 total) |
| MLB, team-only core, v1 (2022–2024; 6,964 games) | No difference | No difference | No difference | **TB-1 better on the result** (+0.004) |
| MLB, team-only core, v2 (team_prior_games 120; TEST 2023–2024; 4,647 games) | **A1 better** (−0.0045) | No difference | **A1 better** | Level on the result; **better on the total at the line** (−0.0013) |
...
**Not validated anywhere yet:** NHL, WNBA, NBL, NRL, rugby union, cricket, NPB, KBO and CPBL. No public results for them could be reached from the implementing session. The code is tested on synthetic data only for these. `python tools/sport_models.py validate --league <key> --from … --to …` runs the same comparison from ESPN, or from a `--csv` file for the Asian baseball leagues.

**What this does and does not show.**
- Across soccer, NFL, AFL and NBA, a pooled team-strength model with a sport-native distribution clearly beats the population baseline on results and margins. In soccer, AFL and NBA it also beats the TB-1 baseline the cards already print.
- Totals are much harder. Outside the NBA, the model adds little or nothing over the league's own total distribution, which matches TB-1's finding of no total resolution in most leagues.
- None of this is evidence about the cards. The cards' own skill is measured by `C-BASELINE-SKILL` and `T-RM1-PROSPECTIVE`, and the models' prospective record by the shadow lanes.

## v2 re-selections (disclosed)

Two v1 routes failed. Each failure was diagnosed with `diagnose.py` (output in `diagnostics.json`). Then one parameter was re-selected on an **earlier** TUNE window with a selection rule declared in advance (`tune_v2.py`):

- **MLB team-only core was overconfident.** v1's P(home win) ranged from 0.08 to 0.90 (SD 0.147). For example, 2023 Oakland v Texas was priced at 0.08, because offence and prevention ratings multiply and 20 prior games did not shrink them enough.
  - Grid: team_prior_games 20, 40, 60, 80, 120, 160. TUNE window: 2022 only. Selection rule: lowest win log loss. Selected: **120**.
  - On 2023–2024, v2 beats A0 on the result and the totals and draws level with TB-1 on the result. v1 did neither.
  - `tools/mlb_model.py` now uses 120 as well (`MLB-A1-shadow/2026-09-26b`).
- **The tennis games route was biased toward close matches.** With independent, identical points, the chain predicted 24.9 games against 23.7 actual, and 45% three-setters against 37%.
  - v2 adds a match-level random effect to the serve-point gap, which keeps the winner probability unchanged.
  - Grid: gap_sd 0, 0.03, 0.06, 0.09, 0.12. TUNE window: 2021–2022. Selection rule: lowest total-games RPS. Selected: **0.09**.
  - On 2023–2026 the games route no longer loses to the population, and it is marginally better at the fixed line.

**The v2 TEST numbers are not independent evidence**, because the v1 failures were seen on those windows before v2 was written. The independent tests are:
- the 2025 MLB season: `python tools/mlb_model.py validate --season 2025`, which needs statsapi;
...
```
python research/sport_models_2026-09-26/validate_public.py            # v1: every dataset (about 3 minutes per dataset)
python research/sport_models_2026-09-26/tune_v2.py --sport mlb --phase tune --value 120   # one v2 cell
...
`validate_public.py` now runs with the v2 constants, so a rerun of its MLB and ATP cells reproduces v2, not v1. v1 is `tools/sport_models.py` at ac6fdc5. Raw downloads are cached in the git-ignored `cache/`. Their URLs and SHA-256 are in `provenance.json`.
